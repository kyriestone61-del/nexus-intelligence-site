import{S,$,bootSession,loadCloud,loadModules,initNav,go,track}from'https://dmdgkjksouhhsuojthav.supabase.co/functions/v1/hlo-app?asset=core.js';
...
function renderBase(){renderOwner();renderDashboard();renderPath();renderAcademy();renderMastery();renderResearch();renderSources();renderAccount()}
async function boot(){initNav();initAuth();await loadModules();await bootSession();if(S.user){await loadCloud();openAfterAuth();await track('session_started',{has_curriculum:!!S.curriculum})}renderBase();window.addEventListener('hlo:view',e=>{const id=e.detail.id;if(['nexus','statecraft','career','trading'].includes(id)&&!S.owner){go('dashboard');return}const fn=renders[id];if(fn)fn()});const id=location.hash.slice(1);if(S.user&&id&&document.getElementById(id))go(id);else if(S.user)go('dashboard')}
boot().catch(e=>{console.error(e);document.body.insertAdjacentHTML('beforeend',`<div style="position:fixed;inset:auto 20px 20px 20px;background:#4a1f1f;color:white;padding:14px;border-radius:10px;z-index:9999">Human OS startup error: ${String(e.message||e)}</div>`)});
