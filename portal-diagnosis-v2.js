...
(function(){
  ...
  document.addEventListener('click',async e=>{
    ...
    const go=e.target.closest?.('.diagnosis-goto');if(go){closeReview({clear:true});if(window.__relystraDeliveryLifecycle){window.NexusAdminJourney?.navigate?.(({tasks:'actions',overview:'overview',timeline:'projects'}[go.dataset.section]||'overview'));return}document.querySelector(`.side-nav button[data-section="${go.dataset.section}"]`)?.click()}
  });
