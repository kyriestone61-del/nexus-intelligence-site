...
assert.match(shell,/PRIMARY_VIEWS=\[\['today','Overview'\],\['reports','Diagnosis'\],\['actions','Actions'\],\['builds','Builds'\],\['progress','Progress'\],\['final-package','Final Package'\],\['support','Support'\]\]/);
assert.match(shell,/lifecycle\(lifecycleStore.value\)/);assert.match(shell,/data-client-go/);assert.match(shell,/Reports/);assert.match(shell,/Help/);assert.match(shell,/Inbox/);
