...
assert.match(app,/isSignedIn&&!isAdmin[\s\S]*portal-client-shell\.css[\s\S]*portal-client-core\.js[\s\S]*portal-client-shell\.js/,'client branch must load consolidated shell');
const clientBranch=app.match(/if\(isSignedIn&&!isAdmin\)\{([\s\S]*?)\}\s*else if\(isSignedIn&&isAdmin\)/)?.[1]||'';
