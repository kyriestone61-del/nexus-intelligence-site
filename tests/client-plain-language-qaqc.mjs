...
assert.match(app,/const BUILD='20260904-[A-Za-z0-9._-]+'/,'client portal must use a current dated cache-busting build id');
...
assert.match(copy,/\[data-prep-build\]/,'Build with Nexus control must receive a clearer client label');
...
console.log('NEXUS CLIENT PLAIN-LANGUAGE QAQC PASS');
