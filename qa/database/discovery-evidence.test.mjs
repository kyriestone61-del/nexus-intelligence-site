...
test('large evidence is fully segmented without truncation and report provenance rejects invented sources',()=>{
 ...
 const supported={...Object.fromEntries(discoverySections.map(k=>[k,[{text:'Supported statement',confidence:'strongly_indicated',source_refs:['s']}]])),executive_summary:[{label:'reviewed',text:'Reviewed evidence',source_refs:['s']},{label:'primary_friction',text:'Primary friction',source_refs:['s']},{label:'overall_opportunity',text:'Overall opportunity',source_refs:['s']},{label:'deeper_investigation',text:'A remaining question',source_refs:['s']}],diagnosis_snapshot:[{area:'Operations',condition:'moderate_friction',priority:'medium',source_refs:['s']}],quick_wins:[{title:'Standardize intake',text:'Use one intake checklist.',potential_benefit:'Improve consistency.',confidence:'strongly_indicated',source_refs:['s']},{title:'Clarify ownership',text:'Record one owner per handoff.',potential_benefit:'Reduce follow-up ambiguity.',confidence:'strongly_indicated',source_refs:['s']}],contradictions:[]};
 const normalized=validateFreeDiagnosis(supported,['s']);assert.equal(normalized.quick_wins.length,2);assert.equal(normalized.diagnosis_snapshot[0].area,'Operations');
 supported.diagnosis_snapshot[0].area='Invented category';assert.throws(()=>validateFreeDiagnosis(supported,['s']),/INVALID_FREE_DIAGNOSIS_SNAPSHOT/);
});
