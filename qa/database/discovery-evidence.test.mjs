...
test('complete hierarchical state machine covers three documents plus > single-prompt evidence and revisions',async()=>{
 ...
 try{
  ...
  let s=await workspace('process');let extracts=0;const deps={db:serviceAdapter(db),config:async()=>({}),hash:async()=> 'test-content-hash',parse:async d=>({text:sources.get(d.id),parser:'text',parsed:true}),call:async(...args)=>{if(args[1]==='Document evidence extractor')extracts++;return deterministicDiscoveryModel(...args)}};
  ...
  s=await workspace();const first=s.reports[0];assert.equal(first.status,'complete');assert.equal(first.report.coverage.documents,4);assert.equal(first.report.coverage.chunks,extracts);assert.equal(first.report.evidence_ledger.length,extracts);
