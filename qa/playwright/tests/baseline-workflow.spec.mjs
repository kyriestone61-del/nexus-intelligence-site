...
async function signIn(page,email,password){
  const alreadySignedIn=await page.evaluate(email=>window.NexusPortal?.state?.user?.email===email,email);
  if(alreadySignedIn){await page.evaluate(()=>window.NexusPortal.workspace());await waitForSettledPortal(page);return}
...
async function openClientActionDetails(card){
...
async function processClientSubmission(adminPage,clientPage,task,{exerciseHelp=false,exerciseRevision=false,fileName=`qa-action-${task.id.slice(0,8)}.csv`}={}){
  ...
  page=adminPage;await adminLogin(page);await openAdminActions(page,'ready_review');
  ...
  if(exerciseRevision){
    ...
    await adminCard.locator('.admin-revise-task').click();await waitForTaskStatus(page,task.id,'needs_revision');
    page=clientPage;await clientLogin(page);await openClientActions(page);card=page.locator(`[data-action-engine-task="${task.id}"]`);await expect(card).toBeVisible();
    const restart=card.locator('[data-action-start]');if(await restart.isVisible().catch(()=>false))await restart.click();
    card=page.locator(`[data-action-engine-task="${task.id}"]`);await openClientActionDetails(card);await fillRequiredActionFields(card);
    await card.locator('[data-action-submit]').click();await waitForTaskStatus(page,task.id,'ready_for_review');
    page=adminPage;await adminLogin(page);await openAdminActions(page,'ready_review');adminCard=page.locator(`.action-v2-card[data-task-id="${task.id}"],.operational-action-card[data-task-id="${task.id}"]`).first();await expect(adminCard).toBeVisible({timeout:20_000});
  }
...
test.describe('full governed Relystra baseline workflow',()=>{
  ...
  test('call evidence → diagnosis → founder selection → routed actions → revision → completion → file upload',async({page,browser},testInfo)=>{
    ...
    const errors=[],captureError=message=>{if(message.type()==='error'&&!/favicon|cloudflareinsights|analytics|ResizeObserver loop/i.test(message.text()))errors.push(message.text())};
