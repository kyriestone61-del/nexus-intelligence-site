...
test('transcript upload failure retains file, retry persists, diagnosis failure recovers without duplicate navigation runs',async({page})=>{
  ...
  await page.getByRole('button',{name:'Run Full Diagnosis',exact:true}).click();await expect(page.getByRole('alert')).toContainText('Simulated diagnosis failure');
  ...
  await expect(page.getByRole('button',{name:'Run Full Diagnosis',exact:true})).toBeDisabled();
...
test('client can save a transcript but cannot run internal diagnosis or bypass unpaid access',async({page})=>{
 ...
 await expect(page.getByRole('button',{name:'Review scope & payment',exact:true})).toBeVisible();await expect(page.locator('[data-transcript-run]')).toHaveCount(0);await expect(page.locator('#calls')).toHaveText('0');
});
