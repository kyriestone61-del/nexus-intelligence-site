...
test('auth tabs and verification entry surface remain stable',async({page})=>{
  ...
  await page.locator('#tabCreate').click();await expect(page.locator('#createForm')).toBeVisible();await expect(page.locator('#createCompany')).toBeVisible();
  await page.locator('#tabSignIn').click();await expect(page.locator('#signInForm')).toBeVisible();
...
test.describe('authenticated client control room',()=>{
  ...
  test('client sees exactly Home Tasks Files Reports with working utilities',async({page})=>{
    const errors=[];page.on('console',message=>{if(message.type()==='error')errors.push(message.text())});
    ...
    expect(await nav.allTextContents()).toEqual(SIMPLE_LABELS);
    await expect(page.locator('#nexus-client-home')).toBeVisible();
    await expect(page.getByText('What needs your attention.',{exact:true})).toBeVisible();
    await nav.nth(1).click();await expect(page.locator('#nexus-client-tasks')).toBeVisible();await expect(page.getByText('Work, without the noise.',{exact:true})).toBeVisible();
    await nav.nth(2).click();await expect(page.locator('#nexus-client-files')).toBeVisible();await expect(page.locator('#uploadForm')).toBeVisible();
    await nav.nth(3).click();await expect(page.locator('#nexus-client-reports')).toBeVisible();
    await expect(page.locator('#nexusClientReportsButton')).toHaveCount(0);
  ...
  test('client portal settles without runaway inbox traffic',async({page})=>{
    let inboxCalls=0;page.on('request',request=>{if(request.url().includes('/rest/v1/rpc/nexus_get_inbox'))inboxCalls+=1;});
    await signIn(page,clientEmail,clientPassword);inboxCalls=0;await page.waitForTimeout(3_000);
    expect(inboxCalls,'Idle client portal must not continuously poll nexus_get_inbox').toBeLessThanOrEqual(4);
    await expect(page.locator('#portalApp')).toBeVisible();await expect(page.locator('body')).not.toHaveClass(/nexus-runtime-booting/);
  });

  test('company selector refreshes one coherent four-surface workspace',async({page})=>{
    await signIn(page,clientEmail,clientPassword);
    ...
    if(optionCount>1){const second=await select.locator('option').nth(1).getAttribute('value');await select.selectOption(second);await expect.poll(async()=>await page.locator('#nexusClientMiniContext b').textContent()).not.toBe('');await expect(page.locator('#nexus-client-home')).toBeVisible();await expect(page.locator('#nexusClientPrimaryNav [data-client-view]')).toHaveCount(4);}
  });
});
...
test.describe('administrator and client-preview boundaries',()=>{
  ...
  test('administrator settles without runaway inbox traffic or boot lock',async({page})=>{
    let inboxCalls=0;const errors=[];page.on('request',request=>{if(request.url().includes('/rest/v1/rpc/nexus_get_inbox'))inboxCalls+=1;});page.on('console',message=>{if(message.type()==='error')errors.push(message.text())});
    await signIn(page,adminEmail,adminPassword);inboxCalls=0;await page.waitForTimeout(3_000);
    expect(inboxCalls).toBeLessThanOrEqual(4);await expect(page.locator('#portalApp')).toBeVisible();await expect(page.locator('body')).not.toHaveClass(/nexus-runtime-booting/);await expect(page.locator('#nexusPortalBootOverlay')).toHaveCount(0);expect(meaningfulConsoleErrors(errors)).toEqual([]);
  });

  test('administrator can enter simplified Client View without boot failure',async({page})=>{
    const errors=[];page.on('console',message=>{if(message.type()==='error')errors.push(message.text())});
    ...
    const switcher=page.locator('#nexusPerspectiveSwitcher');await expect(switcher).toBeVisible();await switcher.locator('summary').click();await switcher.locator('[data-perspective="client"]').click();
    await expect(page.locator('#nexusClientPrimaryNav')).toBeVisible({timeout:25_000});await expect(page.getByText('Nexus could not finish loading.',{exact:true})).toHaveCount(0);
    await expect(page.locator('#nexusClientPrimaryNav [data-client-view]')).toHaveCount(4);expect(await page.locator('#nexusClientPrimaryNav [data-client-view]').allTextContents()).toEqual(SIMPLE_LABELS);expect(meaningfulConsoleErrors(errors)).toEqual([]);
  });
  ...
  test('legacy admin modals open and close without scroll or focus leakage',async({page})=>{
    ...
    for(const [buttonSelector,modalSelector] of cases){const button=page.locator(buttonSelector);if(!(await button.isVisible().catch(()=>false)))continue;await button.focus();await button.click();const modal=page.locator(modalSelector);await expect(modal).toHaveClass(/show/);await expect(page.locator('body')).toHaveClass(/nexus-modal-open/);await page.keyboard.press('Escape');await expect(modal).not.toHaveClass(/show/);await expect(page.locator('body')).not.toHaveClass(/nexus-modal-open/);await expect(button).toBeFocused();}
  });
});
