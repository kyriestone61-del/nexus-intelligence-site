...
function render(){
  try{
    ...
    if($('activityList'))$('activityList').innerHTML=state.activity.map(item=>`<div class="row"><div class="grow"><b>${esc(item.summary||item.action)}</b><div class="small">${esc(item.entity_type||'activity')}</div></div><div class="small">${dt(item.created_at)}</div>`).join('')||'<div class="empty">No activity yet.</div>';
