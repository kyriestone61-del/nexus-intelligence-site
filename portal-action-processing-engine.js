...
if(!portal)throw new Error('Relystra portal context is unavailable for Action Item Processing Engine.');
...
function ownerLabel(task){if(task.owner_user_id&&task.owner_user_id===state.user?.id)return 'You';return norm(task.owner_scope||task.assignee)==='client'?'Client':'Relystra'}
function statusLabel(status){return ({draft:'Draft',open:'Not started',not_started:'Not started',waiting_on_client:'Waiting on client',in_progress:'In progress',blocked:'Blocked',ready_for_review:'Ready for Relystra review',needs_revision:'Changes requested',approved:'Approved',completed:'Completed',done:'Completed',not_applicable:'Not applicable'}[norm(status)]||String(status||'Not started').replaceAll('_',' '))}
...
function clientTaskCard(task){
  ...
  return `<article class="action-engine-card ${group}" data-action-engine-task="${esc(task.id)}">
    ...
    ${task.review_note?`<div class="action-engine-revision"><b>${norm(task.status)==='needs_revision'?'Changes requested by Relystra':'Review note'}</b><p>${esc(task.review_note)}</p></div>`:''}
    <div class="action-engine-details" hidden>
      ...
      ${actionable?`<label class="action-engine-note"><span>Note for Relystra <small>(optional)</small></span><textarea data-action-note placeholder="Add context that will help Relystra review this action.">${esc(task.response_data?.client_note||'')}</textarea></label>`:''}
      ${historyMarkup(task)}
    </div>
    ...
    <div class="action-engine-inline-box" data-help-box hidden><textarea placeholder="Tell Relystra what is blocking you or what you need help with."></textarea><button class="btn secondary" type="button" data-send-help>Send help request</button></div>
    <div class="action-engine-actions">
      ...
      ${actionable?'<button class="btn secondary" type="button" data-action-help>Request help</button><button class="btn primary" type="button" data-action-submit>Submit to Relystra →</button>':''}
...
function renderClientActions(){
  ...
  const labels={needs_you:'Required now',with_nexus:'With Relystra',upcoming:'Blocked / upcoming',completed:'Completed'};
  ...
  const markup=`<header class="relystra-step-page-head action-engine-page-head" data-step-page="actions"><div class="relystra-step-page-copy"><div class="eyebrow">Step 5 of 11 · Required Inputs</div><h1>Required Inputs</h1><p>See exactly what Relystra still needs, what is complete, what is missing, and which items block Build planning.</p></div><aside class="relystra-step-state" aria-label="Step status"><span>Your required inputs</span><strong>${counts.needs_you} missing · ${counts.completed} complete</strong><small>Next · Recommended Builds</small></aside></header>
    <section class="relystra-input-summary" aria-label="Required input status"><div><span>Required now</span><strong>${counts.needs_you}</strong><small>Answer, upload, or decide</small></div><div><span>With Relystra</span><strong>${counts.with_nexus}</strong><small>Submitted for review</small></div><div><span>Blocked / upcoming</span><strong>${counts.upcoming}</strong><small>Waiting on a prerequisite</small></div><div><span>Completed</span><strong>${counts.completed}</strong><small>Preserved in the record</small></div></section>
...
function scheduleAdminDecoration(){if(!state.admin)return;clearTimeout(adminDecorateTimer);adminDecorateTimer=setTimeout(()=>{loadHistory().then(decorateAdminCards).catch(error=>console.error('Relystra action audit history failed to load.',error))},140)}
...
window.addEventListener('nexus:workspace-ready',async event=>{
  ...
  try{await loadHistory(true)}catch(error){console.error('Relystra action history could not refresh.',error)}
...
if(state.user){loadHistory().catch(error=>console.error('Relystra action history could not load.',error)).finally(()=>{if(state.admin){enhanceBaseTaskModal();scheduleAdminDecoration()}else{ensureClientActionSurface();renderClientActions()}})}
