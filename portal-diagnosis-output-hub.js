...
const LABELS={metrics:'Metrics',projects:'Project',approvals:'Approvals',milestones:'Milestones',nexus_tasks:'Nexus Tasks',opportunities:'Opportunities',client_actions:'Client Actions',document_requests:'Document Requests'};
...
function renderNexusTasks(){return data.nexus_tasks?.length?`<div class="output-list">${data.nexus_tasks.map(x=>`<article class="output-card output-check-card ${done(x.status)?'complete':''}"><button class="output-check" data-complete-task="${esc(x.id)}" type="button" ${done(x.status)?'disabled':''} aria-label="${done(x.status)?'Completed':'Mark complete'}">${done(x.status)?'✓':''}</button><div><div class="output-card-top"><h3>${esc(x.title)}</h3>${statusPill(x.status)}</div><p>${esc(x.description||'')}</p><small>${esc(x.priority||'normal')} priority${x.due_date?` · due ${esc(x.due_date)}`:''}</small></div></article>`).join('')}</div>`:'<div class="empty">No Nexus-owned diagnosis tasks were generated.</div>'}
...
async function completeTask(id,button){
  ...
  toast?.('Nexus task completed.');await workspace?.();await loadOutputs(currentRun.id);render();
}
