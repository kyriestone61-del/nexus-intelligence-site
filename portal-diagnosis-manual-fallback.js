...
async function enhance(id=currentRunId){
 ...
 panel.innerHTML=`<div class="kicker">Continue without the automatic provider</div><h3 style="margin:6px 0 8px">Finish Full Diagnosis with ChatGPT</h3><p class="small">Your evidence and admin context are still saved. Use this governed fallback only while the automatic diagnosis provider is unavailable.</p><div class="actions">${evidenceIds.slice(0,8).map((id,i)=>`<button class="btn secondary" data-manual-download-doc="${esc(id)}" type="button">Evidence ${i+1}</button>`).join('')}<button class="btn secondary" data-manual-copy="${esc(id)}" type="button">Copy diagnosis prompt</button></div><div class="field" style="margin-top:12px"><label>Paste the completed diagnosis JSON</label><textarea data-manual-result="${esc(id)}" rows="9" placeholder="Paste the diagnosis output from ChatGPT here..."></textarea></div><div class="actions"><button class="btn primary" data-manual-save="${esc(id)}" type="button">Save diagnosis for review →</button></div><p class="small">After saving, Relystra moves the diagnosis to <b>Ready for Review</b>. Review the structured findings and approve them to complete Full Diagnosis and unlock planning.</p>`;
}
