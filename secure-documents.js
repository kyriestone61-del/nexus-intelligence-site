export async function init({sb,state,$,toast,download,log,workspace}){
  ...
  section.innerHTML=`
    <div class="toolbar"><div><div class="eyebrow">Secure Files & Requests</div><h1 style="font-size:36px;margin:6px 0">Documents</h1><p class="muted" style="margin:6px 0 0">Upload only information that is relevant to the work. Relystra will request specific documents whenever possible so you do not have to guess what is needed.</p></div><div id="secureAdminBar" class="secure-admin-bar"></div></div>
    <div class="secure-docs-intro">
      <div class="secure-guide good"><h3>✓ Good to upload</h3><p>Normal business information that helps Relystra understand or improve a process.</p><ul><li>SOPs and process documents</li><li>Business plans and operating models</li><li>Templates, reports and dashboards</li><li>Org charts, policies and workflow screenshots</li></ul></div>
      <div class="secure-guide caution"><h3>△ Confidential — only when needed</h3><p>Upload only when relevant to the engagement, and redact details Relystra does not need.</p><ul><li>P&Ls, budgets and forecasts</li><li>Internal pricing and strategy</li><li>Contracts and customer lists</li><li>CRM exports and internal performance reports</li></ul></div>
    ...
    <div class="secure-doc-grid">
      <div class="secure-doc-section"><h2>Requested from You</h2><p class="section-copy">Specific information Relystra needs, why it is needed, what a good file looks like, and what should be removed first.</p><div id="requestedDocs"></div></div>
      <div class="secure-doc-section"><h2>Shared with You</h2><p class="section-copy">Reports, revised files, deliverables and other documents Relystra has returned to your company.</p><div id="sharedDocs"></div></div>
      ...
      <div class="secure-doc-section"><h2>Upload Other File</h2><p class="section-copy">Use this when a file is relevant but Relystra has not specifically requested it.</p>
        <div class="secure-upload"><form id="secureUploadForm">
          <div class="form-grid"><div class="field"><label>File</label><input id="secureDocFile" type="file" required accept=".pdf,.docx,.xlsx,.csv,.txt,.png,.jpg,.jpeg"></div><div class="field"><label>Fulfills a Relystra request?</label><select id="secureRequest"><option value="">No — general upload</option></select></div></div>
          ...
          <label class="secure-policy-check"><input id="secureConfirm" type="checkbox" required><span>I confirm this file is relevant to Relystra work and does not contain passwords, API secrets, MFA codes, full payment-card data, SSNs, medical records, crypto private keys, or other restricted information that Relystra has not expressly approved.</span></label>
          <button class="btn primary" type="submit">Upload securely</button><p class="small">PDF, DOCX, XLSX, CSV, TXT, PNG or JPG · Maximum 25 MB · Access is limited to the authenticated company workspace and authorized Relystra administrators.</p>
        </form></div>
  ...
  function renderRequests(){
    ...
    box.innerHTML=visible.length?visible.map(r=>'<div class="request-card '+(r.status==='received'?'received':'')+'"><div class="request-meta"><span class="pill">'+esc(r.status)+'</span><span class="sensitivity '+esc(r.sensitivity)+'">'+(r.sensitivity==='confidential'?'Confidential':'Standard')+'</span><span class="pill">'+esc(fmtDate(r.due_date))+'</span></div><h3>'+esc(r.title)+'</h3>'+(r.purpose?'<div class="small"><b>Why Relystra needs it:</b> '+esc(r.purpose)+'</div>':'')+(r.examples?'<div class="small"><b>Good examples:</b> '+esc(r.examples)+'</div>':'')+(r.redaction_guidance?'<div class="small"><b>Remove / redact first:</b> '+esc(r.redaction_guidance)+'</div>':'')+'<div class="actions" style="margin-top:10px">'+(r.status==='requested'?'<button class="btn primary upload-for-request" data-id="'+r.id+'" type="button">Upload for this request</button>':'')+(state.admin?'<select class="secure-status request-status-admin" data-id="'+r.id+'"><option value="requested" '+(r.status==='requested'?'selected':'')+'>Requested</option><option value="received" '+(r.status==='received'?'selected':'')+'>Received</option><option value="waived" '+(r.status==='waived'?'selected':'')+'>Waived</option><option value="cancelled" '+(r.status==='cancelled'?'selected':'')+'>Cancelled</option></select>':'')+'</div></div>').join(''):'<div class="secure-empty">No document requests right now. Relystra will place specific requests here when information is needed.</div>';
  ...
  function renderSecure(){
    ...
    $('sharedDocs').innerHTML=shared.length?shared.map(fileRow).join(''):'<div class="secure-empty">No Relystra-shared documents yet.</div>';
