export async function initOps({sb,state,$,toast,workspace,log}){
  ...
  function buildNav(){
    ...
    if(state.admin){
      group(nav,'Relystra Admin');nav.appendChild(customButton('Command Center','command'));nav.appendChild(customButton('Clients','clients'));
  ...
  function roleUI(){
    ...
    if($('opsRoleChip'))$('opsRoleChip').textContent=state.admin?'Relystra Admin':'Client Workspace';
  ...
  function renderToday(){
    ...
    root.innerHTML=`<div class="ops-hero"><div class="eyebrow">${state.admin?'Client workspace':'Today'}</div><h1>${state.admin?esc(c?.name||'Client'):'What needs your attention?'}</h1><p>${state.admin?'Review current client work, decisions, systems, and measured value in one place.':'This is your Relystra action center. Start here instead of hunting through tabs.'}</p></div>
