...
export const deliverySections=[['overview','Overview'],['diagnosis','Diagnosis'],['actions','Actions'],['builds','Builds'],['progress','Progress'],['final-package','Final Package'],['support','Support']];
export function lifecycle(snapshot){
  ...
  if(d.status!=='approved'){
    if(!d.access)return result('diagnosis_purchase','CLIENT','Start with an operational diagnosis','After the fit call, purchase the diagnosis to begin the structured evidence review.','diagnosis','Review diagnosis offer');
  ...
  if(s.builds?.approved)return result('build_selection','CLIENT','Choose your Builds','Select the approved recommendations to review one fixed-price Build Plan.','builds','Choose Builds');
