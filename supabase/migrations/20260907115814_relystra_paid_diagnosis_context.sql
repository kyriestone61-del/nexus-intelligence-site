...
begin
  ...
  -- A new paid package can use a diagnosis from a retained historical project.
  -- Resolve its immutable purchased references before considering newer work.
  select r.* into d from public.nexus_diagnosis_runs r
    where r.company_id=p_company_id and r.status<>'draft'
      and exists(select 1 from jsonb_array_elements(coalesce(p.scope_snapshot->'items','[]')) item
        where item->>'diagnosis_run_id'=r.id::text)
