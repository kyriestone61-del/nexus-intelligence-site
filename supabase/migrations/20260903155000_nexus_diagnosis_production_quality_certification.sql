...
begin
  ...
  return jsonb_build_object('score',least(100,greatest(0,v_score)),'passed',v_score>=85,'issues',to_jsonb(v_issues),'rubric_version','2026-09-03-v1','threshold',85,'source','production');
exception when others then
  return jsonb_build_object('score',0,'passed',false,'issues',jsonb_build_array('Structural certification failed: '||sqlerrm),'rubric_version','2026-09-03-v1','threshold',85,'source','production');
end
$$;
...
begin
  ...
  v_case_ref:='PROD-RUN:'||new.id::text||':ATTEMPT:'||coalesce(new.execution_attempts,0)::text;
  if not exists(select 1 from public.nexus_agent_evaluations where agent_code='client_diagnosis' and case_type='regression' and case_ref=v_case_ref) then
...
as
select e.id,e.case_ref,e.score,e.passed,e.failure_class,e.intervention_required,e.notes,e.evaluated_at
from public.nexus_agent_evaluations e
where e.agent_code='client_diagnosis' and e.case_type='regression' and e.case_ref like 'PROD-RUN:%'
...
select
  'client_diagnosis',
  'regression',
  'PROD-RUN:'||r.id::text||':ATTEMPT:'||coalesce(r.execution_attempts,0)::text,
...
where r.analysis_result is not null
  ...
  and not exists(
    ...
    where e.agent_code='client_diagnosis' and e.case_type='regression'
      and e.case_ref='PROD-RUN:'||r.id::text||':ATTEMPT:'||coalesce(r.execution_attempts,0)::text
  );
