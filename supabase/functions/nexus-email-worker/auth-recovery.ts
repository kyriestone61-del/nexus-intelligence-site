...
export function isAuthInvite(row:any){
  return row?.message_kind===INVITE_KIND&&row?.related_type==='auth_email_event'&&typeof row?.related_id==='string';
}
...
function firstPartyAuthLink(actionLink:string,expectedType:string,mode:string){
  try{
    const generated=new URL(actionLink);
    const tokenHash=generated.searchParams.get('token');
    const type=generated.searchParams.get('type')||expectedType;
    if(!tokenHash||type!==expectedType)return null;
    const target=new URL('/portal',PUBLIC_ORIGIN);
    target.hash=new URLSearchParams({mode,type,token_hash:tokenHash}).toString();
...
export async function prepareAuthRecovery(row:any,baseUrl:string,headers:Record<string,string>){
  ...
  const recoveryLink=actionLink?firstPartyAuthLink(actionLink,'recovery','recovery'):null;
...
export async function prepareAuthInvite(row:any,baseUrl:string,headers:Record<string,string>){
  if(!isAuthInvite(row))return null;
  const response=await fetch(`${baseUrl}/auth/v1/admin/generate_link`,{
    method:'POST',headers,body:JSON.stringify({type:'magiclink',email:String(row.recipient_email||'').trim().toLowerCase(),redirect_to:`${PUBLIC_ORIGIN}/portal`})
  });
  const payload=await response.json().catch(()=>({}));
  const actionLink=payload?.action_link||payload?.properties?.action_link||null;
  const inviteLink=actionLink?firstPartyAuthLink(actionLink,'magiclink','invite'):null;
  if(!response.ok||!inviteLink){
    await patchEvent(baseUrl,headers,row.related_id,{status:'internal_failed',error_code:response.ok?'invalid_invite_action_link':`generate_link_${response.status}`,metadata:{delivery_path:'nexus_email_outbox',token_persisted:false,generation_failed_at:new Date().toISOString()}});
    throw new Error(`AUTH_INVITE_LINK_${response.status}`);
  }
  await patchEvent(baseUrl,headers,row.related_id,{status:'generated',error_code:null,metadata:{delivery_path:'nexus_email_outbox',token_persisted:false,first_party_invite_link:true,generated_at:new Date().toISOString()}});
  return `${clean(row.body_text,4000)}\n\nOpen your secure workspace: ${inviteLink}\n\nThis one-time link signs you in to the client company selected by Relystra. Do not forward this email.`;
}
...
export async function markAuthRecoveryProviderFailure(row:any,baseUrl:string,headers:Record<string,string>,status:number,errorMessage:string){
  if(!isAuthRecovery(row)&&!isAuthInvite(row))return;
  await patchEvent(baseUrl,headers,row.related_id,{
