...
async function notifyAdminsPacket(lead:any,packetId:string){const admins=await rest('nexus_platform_admins?select=user_id').catch(()=>[]);if(!admins.length)return;await insert('nexus_notifications',admins.map((a:any)=>({user_id:a.user_id,notification_type:'outreach_packet_ready',title:'Outreach packet ready for review',message:`${clean(lead.company_name,120)} scored ${lead.opportunity_score}. Review the evidence-backed packet before any outreach is sent.`,related_type:'outreach_packet',related_id:packetId,action_url:'/portal?view=revenue-engine'})),'return=minimal').catch(()=>{})}
...
async function processRevenueJob(job:any,cfg:any){
  ...
  const evidencePlan=await model(cfg,'Evidence Strategist',`Choose the strongest verified personalization hook and the strongest operational gap. Determine whether any numeric lost-revenue statement is usable (only when a stored evidence-backed estimate and basis exist). Determine whether a Nexus proof metric is usable (only from the supplied publishable_proof array). Return keys: hook_evidence_ids, hook_statement, gap_evidence_ids, gap_statement, allowed_economic_statement, allowed_nexus_proof_statement, missing_information, outreach_blockers. Do not draft the emails.`,{lead,verified_evidence:allEvidence,publishable_proof:proof,open_exceptions:exceptions});
  if(Array.isArray(evidencePlan?.outreach_blockers)&&evidencePlan.outreach_blockers.length){await addException(lead.id,'outreach_evidence_blocked','high','Outreach Evidence Strategist found blockers.',{blockers:evidencePlan.outreach_blockers});await patch('nexus_revenue_agent_jobs',job.id,{status:'blocked',completed_at:now(),error:'OUTREACH_EVIDENCE_BLOCKED',result:{evidence_plan:evidencePlan}});return {status:'blocked',reason:'OUTREACH_EVIDENCE_BLOCKED'}}

  const draft=await model(cfg,'Hyper-Personalized Outreach Drafter',`Create a high-quality outreach packet. The teardown should read naturally in 30-60 seconds. Email 1 must be direct, specific, respectful and non-pushy; focus on the verified operational/economic gap, not generic AI hype. Email 2 is a follow-up intended for 3 days after Email 1 is actually sent and should introduce the custom Nexus Snapshot preview. Offer a 2-minute workflow map / Snapshot next step, but do not claim it already exists unless snapshot_preview actually provides the described concept. Return exactly: teardown_script, email_1:{subject,body}, email_2:{subject,body}, snapshot_preview:{headline,observed_gap,current_flow,proposed_flow,expected_operational_effect,cta}, claim_map:[{claim,evidence_ids,claim_type}], confidence (0-100), compliance_flags:[string], generation_notes:{proof_metric_used:boolean,lost_revenue_used:boolean}.`,{lead,evidence_plan:evidencePlan,verified_evidence:allEvidence,publishable_proof:proof});

  const verify=await model(cfg,'Independent Outreach QA / Governance Verifier',`Audit the draft independently. Fail any fabricated observation, unsupported number, fake Nexus proof metric, implied completed form test without evidence, invented decision maker, unsupported revenue-loss figure, spammy/manipulative wording, missing evidence mapping, do-not-contact conflict, personal-contact provenance concern, or language implying that outreach/automation already happened. Return: pass (boolean), quality_score (0-100), issues:[{severity,problem,required_correction}], release_blockers:[string].`,{lead,verified_evidence:allEvidence,publishable_proof:proof,draft});
  ...
  await patch('nexus_revenue_leads',lead.id,{stage:'outreach_ready',suggested_action:'Review evidence-backed outreach packet; approve or revise before any external contact.'});
  await patch('nexus_revenue_agent_jobs',job.id,{status:'completed',completed_at:now(),result:{packet_id:packet.id,version,qa_score:Number(verify?.quality_score||0),qa_pass:verify?.pass===true,evidence_count:allEvidence.length}});
  await notifyAdminsPacket(lead,packet.id);
  return {status:'completed',packet_id:packet.id,qa_score:Number(verify?.quality_score||0),qa_pass:verify?.pass===true}
}
