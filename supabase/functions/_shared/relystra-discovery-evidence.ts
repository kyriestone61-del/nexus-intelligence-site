...
export function applyFreeDiagnosisReview(draft:any,review:any,sourceIds:string[]){
 ...
 for(const correction of review.corrections){
  if(![...discoverySections,'contradictions'].includes(correction?.section)||seen.has(correction.section)||!Array.isArray(correction.items))throw new Error('INVALID_FREE_DIAGNOSIS_CORRECTION');
