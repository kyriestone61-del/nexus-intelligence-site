...
export async function discoveryWork(deps:Dependencies,engagementId:string|null=null){
 ...
 try{
  ...
  const review=await deps.call(cfg,'Independent source fidelity and contradiction reviewer',`Review the complete draft against all supplied evidence. Return ONLY {corrections:[{section,items}],qa:{pass:boolean,issues:[string]}}. Corrections replace ONLY changed report sections; use [] when the draft is sound. Allowed sections: business_context,current_processes,observed_problems,key_findings,opportunity_areas,missing_information,evidence_confidence,executive_summary,diagnosis_snapshot,quick_wins,contradictions. Preserve each section's existing structured item schema. Do not rewrite or return the complete report. Correct unsupported claims, overlooked conflicts, fabricated benefits, unsupported priorities and overconfidence. Missing business information is an honest gap, not a report defect when disclosed. An inference is acceptable when labeled. Keep the Free Diagnosis preliminary: remove detailed implementation plans, architecture, pricing, schedules, ROI claims, and unsupported root-cause claims. Set pass true only when no report-quality defects remain after applying corrections; issues lists ONLY unresolved defects. Never invent new evidence. Keep corrections concise, preferably under 7000 characters.`,{draft:job.draft,evidence:job.nodes,previous_unresolved_issues:job.draft?._qa_previous_issues||[]},0.05,90000);
