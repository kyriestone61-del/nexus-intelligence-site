...
async function recommendBuilds(req:Request,companyId:string,runId:string,catalogAfter=''){
  ...
  const [diagnosis,actions,catalog]=await Promise.all([
    ...
    db.from('nexus_resolution_catalog').select('code,title,default_recipe').eq('active',true).eq('default_recipe->>catalog_kind','build_template').gt('code',catalogAfter).order('code').limit(6),
  ]);
  ...
  const templates=catalog.data||[];if(!templates.length)return {ok:true,build_ids:[],next_cursor:null,reviewed_template_codes:[],human_review_required:true};
  ...
  const builds=await validatedBuildRecommendations((correction,timeoutMs)=>callJson(cfg,'Evidence-backed Build recommendations',
    ...
    {findings,accepted_inputs:inputs,build_template_catalog:qualificationCatalog(templates),allowed_references:{source_paths:findings.map(f=>f.source_path),template_codes:templates.map(t=>t.code),action_ids:inputs.map(i=>i.id)},correction},0.04,timeoutMs),findings,templates,inputs);
  const actor=createClient(url,anon,{global:{headers:{Authorization:req.headers.get('authorization')||''}},auth:{persistSession:false,autoRefreshToken:false}});
  ...
  if(error)throw error;
  return {ok:true,build_ids:data||[],status:'proposed',next_cursor:templates.length===6?templates.at(-1)!.code:null,reviewed_template_codes:templates.map((t:any)=>t.code),human_review_required:true};
}
