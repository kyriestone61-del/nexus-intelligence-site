...
async function evidence(doc:any,cfg:any){
  ...
  if(mime.includes("pdf")||name.endsWith(".pdf")){
    ...
    text=await pptxText(bytes);parser="pptx";
  }else if(/\.(xlsx|xls)$/i.test(name)||mime.includes("spreadsheet")||mime.includes("excel")){
    text=await xlsxText(bytes);parser="xlsx";
  }else if(mime.startsWith("image/")||/\.(png|jpg|jpeg|webp|gif)$/i.test(name)){
    text=await imageText(cfg,bytes,mime,doc.file_name);parser="vision";
  }else if(mime.startsWith("text/")||mime.includes("csv")||mime.includes("json")||mime.includes("xml")||/\.(txt|srt|vtt|csv|json|md|xml|html|htm)$/i.test(name)){
...
async function clientResponseEvidence(companyId:string,projectId:string|null){
  let q=db.from("nexus_tasks").select("id,title,form_schema,response_data,status,updated_at,project_id").eq("company_id",companyId).eq("task_type","discovery_information_request").order("updated_at",{ascending:true});
...
async function ingestEvidence(cfg:any,documentId:string){
  ...
  const classification=await callJson(cfg,"Evidence Classifier",
    `Classify this single business evidence source. Identify what kind of source it is, what business domains it can directly support, what it explicitly proves, and what it does not prove. Output exactly: {document_type,domains:[string],summary,claims:[{statement,evidence_ref}],coverage_hints:[string],limitations:[string]}. coverage_hints may contain Master Discovery Framework requirement codes only when the evidence materially addresses them.`,
    {evidence_id:doc.id,file_name:doc.file_name,category:doc.category,note:doc.note,content:safe(parsed.text,140000)});
  const summary=safe(classification?.summary,12000);
  const claims=arr(classification?.claims);
  const {error:updateError}=await db.from("nexus_documents").update({evidence_parser:parsed.parser,evidence_summary:summary||null,evidence_claims:claims,evidence_classification:classification||{},evidence_ingested_at:new Date().toISOString()}).eq("id",doc.id);
  if(updateError)throw updateError;
  return {ok:true,document_id:doc.id,parser:parsed.parser,classification};
}

function validateGap(x:any){
  if(!Array.isArray(x?.requirements)||!Array.isArray(x?.gaps))throw new Error("INVALID_GAP_ANALYSIS");
  if(typeof x?.sufficient_for_diagnosis!=="boolean")throw new Error("INVALID_GAP_SUFFICIENCY");
  if(typeof x?.coverage_score!=="number")throw new Error("INVALID_GAP_COVERAGE");
  return x;
}
async function runGapAnalysis(cfg:any,companyId:string,projectId:string|null,userId:string|null){
  ...
  const gap=validateGap(await callJson(cfg,"Discovery Coverage Analyst",
    `Evaluate the authorized evidence against the reusable Master Discovery Framework. For every framework requirement, mark status answered, partial, missing, or not_applicable. An item is answered only when evidence actually supports it. Do not ask the client to repeat information already present. Output exactly: requirements:[{code,status,confidence,evidence_refs,reason}], gaps:[{code,domain,question,reason,request_kind,desired_evidence,material,priority,document_title,redaction_guidance}], sufficient_for_diagnosis:boolean, coverage_score:number, summary:string. request_kind is question, document, or both. Only partial/missing requirements that could materially change the diagnosis belong in gaps. coverage_score is 0-100. sufficient_for_diagnosis may be true with known unknowns when the evidence is adequate to make a useful bounded diagnosis.`,
    {framework,authorized_evidence:safe(bundle.text,600000)}));
...
function validate(x:any){
  for(const k of ["claims","facts","client_statements","admin_context","inferences","estimates","unknowns","evidence","process_map","bottlenecks","root_causes","baseline_gaps","baseline_measurements","opportunity_backlog","risks","follow_up_questions","nexus_actions","client_action_items","document_requests","decision_items"])
...
document_requests:[{title,purpose,examples,redaction_guidance,sensitivity}], decision_items:[{title,description}], executive_summary:string.
Sensitivity must be standard or confidential. Scores are integers 1-5. Use null for a template_code when no reusable template actually fits.`;
...
async function runPipeline(cfg:any,context:any,evidenceText:string,reviewNote:string){
  const evidencePass=await callJson(cfg,"Evidence Analyst",
    `Build a provenance-first evidence ledger. Extract only source-supported FACTS, CLIENT STATEMENTS, ADMIN CONTEXT, explicit numeric ESTIMATES supplied by a human, UNKNOWNs, contradictions, measurable signals, and process signals. Every supported item must cite an Evidence ID, CLIENT_RESPONSE id, ADMIN_CONTEXT id, or PASTED_TRANSCRIPT. Do not convert an admin observation into a verified fact. Output keys: facts, client_statements, admin_context, estimates, unknowns, contradictions, metric_signals, process_signals, evidence_quality, evidence_ledger.`,
  ...
  const analysisPass=await callJson(cfg,"Process & Opportunity Analyst",
    `Using the evidence ledger, diagnose the current operating system. Build the current state and an end-to-end process map using Trigger → Owner → Inputs → Steps → Systems → Handoffs → Delays → Exceptions → Output. Identify bottlenecks and root causes separately. Establish baselines without inventing numbers. Rank opportunities on impact, feasibility, cost, time-to-value, risk, readiness, and evidence confidence. Use the supplied reusable action-template catalog: select a valid template_code when a standard action pattern fits, then customize its title/description to this client. Use null template_code only when no template fits. Design the smallest sensible first intervention. Output keys: current_state, inferences, process_map, bottlenecks, root_causes, baseline_gaps, baseline_measurements, opportunity_backlog, risks, follow_up_questions, smallest_safe_pilot, recommended_first_intervention, nexus_actions, client_action_items, document_requests, decision_items, executive_summary.`,
    {context,evidence_ledger:evidencePass});

  const verificationPass=await callJson(cfg,"Independent QA / Governance Verifier",
    `Audit the proposed diagnosis independently. Identify unsupported claims, invented numbers, wrong claim types, contradictions silently collapsed, evidence references that do not exist, causal/ROI overclaims, recommendations that do not follow from evidence, unsafe autonomy, missing human controls, template codes not present in the supplied action-template catalog, and duplicated/noisy recommendations. Do not rewrite the report. Output keys: pass (boolean), quality_score (0-100), issues:[{severity,section,problem,required_correction}], remove_claims:[string], required_additions:[string], ranking_corrections:[{title,reason}], release_blockers:[string].`,
  ...
  const finalPass=await callJson(cfg,"Final Diagnosis Composer",
    `Compose the final human-review draft. Apply every verifier correction. Preserve explicit provenance and the six claim classes. Unknowns stay unknown. Keep Current State, Process Map, Evidence, Bottlenecks, Root Causes, Baseline, Opportunities & Priorities, Unknowns, and Recommended First Intervention clear and operational. Recommendations must be proportionate to evidence. Baseline measurements may be null/unknown rather than fabricated. ${schema}`,
    {context,evidence_ledger:evidencePass,analysis:analysisPass,verification:verificationPass},0.05);

  const result=validate(finalPass);
  return {result,verification:verificationPass,stages:{evidence:evidencePass,analysis:analysisPass}};
}

async function actionTemplateCatalog(){
  const {data,error}=await db.from("nexus_action_templates").select("code,title,description,instructions,assignee,priority,task_type,phase,form_schema").eq("active",true).limit(120);
  if(error)return [];
  return (data||[]).map((t:any)=>({code:t.code,title:t.title,description:t.description,assignee:t.assignee,priority:t.priority,task_type:t.task_type,phase:t.phase,instructions:safe(t.instructions,1200),form_schema:t.form_schema}));
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(operation==="ingest_evidence"){
      ...
      return new Response(JSON.stringify(await ingestEvidence(cfg,documentId)),{headers:jh});
    }
    if(operation==="gap_analysis"){
      ...
      const cfg=await providerConfig();
      return new Response(JSON.stringify(await runGapAnalysis(cfg,companyId,projectId,authState.userId)),{headers:jh});
    }
    ...
    const projectId=run.project_id||packet.project?.id||null;
    ...
    const context={
      company:packet.company||{},project:packet.project||{},meeting:packet.meeting||{},
      evidence_manifest:bundle.docs.map((d:any)=>({id:d.id,file_name:d.file_name,category:d.category,note:d.note})),
      action_template_catalog:templates,
      discovery_framework_version:FRAMEWORK_VERSION
    };
    ...
    result.execution={
      ...
      evidence_document_ids:bundle.docs.map((d:any)=>d.id),
      evidence_files:bundle.docs.map((d:any)=>d.file_name),
      evidence_parsers:bundle.parsers,
      admin_context_id:bundle.adminContext?.id||null,
      client_response_refs:bundle.clientResponses,
      action_template_catalog_size:templates.length,
      completed_at:new Date().toISOString(),
      model:MODEL,
      human_review_required:true,
      trigger:authState.mode
    };
    ...
    await health("healthy","Governed evidence-backed Client Diagnosis pipeline completed successfully.",{run_id:runId,pipeline_version:3,qa_score:result.execution.qa_score,qa_pass:result.execution.qa_pass,evidence_count:bundle.docs.length,attempt});
    return new Response(JSON.stringify({ok:true,run_id:runId,status:"ready_for_review",evidence_count:bundle.docs.length,pipeline_version:3,qa_score:result.execution.qa_score}),{headers:jh});
  }catch(e){
