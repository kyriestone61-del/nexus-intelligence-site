...
const validPhone=(v:any)=>/^\+[1-9][0-9]{7,14}$/.test(String(v||"").trim());
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(!sid||!token||!from){
      ...
      await db.from("nexus_sms_outbox").update({status:"unavailable",last_error:"SMS_PROVIDER_NOT_CONFIGURED",provider_status:"unavailable",last_attempt_at:now,updated_at:now}).eq("status","queued");
    ...
    let sent=0,retried=0,failed=0,unavailable=0;
    for(const row of rows||[]){
      if(!validPhone(row.recipient_phone)){
        await db.from("nexus_sms_outbox").update({status:"unavailable",last_error:"INVALID_PHONE_FORMAT",provider_status:"unavailable",updated_at:new Date().toISOString()}).eq("id",row.id);
      ...
      try{
        ...
        const params=new URLSearchParams({To:String(row.recipient_phone).trim(),From:from,Body:body});
    ...
    await health(failed||unavailable?"degraded":"healthy",`SMS worker processed ${(rows||[]).length}; sent ${sent}; retrying ${retried}; unavailable ${unavailable}; failed ${failed}.`,{processed:(rows||[]).length,sent,retried,unavailable,failed});
    return new Response(JSON.stringify({ok:true,processed:(rows||[]).length,sent,retried,unavailable,failed}),{headers:json});
  }catch(e){
