...
function capturedDiscovery(){return diagnosisRuns.find(run=>run.status==='draft')||null}
...
async function loadJourneyData(){
 ...
 const {data,error}=await sb.from('nexus_diagnosis_runs').select('id,project_id,status,created_at,meeting_date,analysis_result').eq('company_id',state.companyId).eq('project_id',activeProject.id).order('created_at',{ascending:false}).limit(20);
...
function nextMove(stage,status){
 ...
 if(stage.key==='discovery'){
   if(capturedDiscovery())return {title:'Complete the captured discovery packet',copy:'The meeting record is safely linked to this client engagement. Add the transcript and supporting evidence, then queue the secured diagnosis when the packet is ready.',label:'Continue Discovery Intake',open:'intake'};
   return {title:'Capture the discovery meeting',copy:'Open Discovery Intake, record the meeting date, participants, notes, or transcript, then use Capture discovery context so it is saved to this client engagement.',label:'Open Discovery Intake',open:'intake'};
 }
 if(stage.key==='diagnosis'){
...
function renderJourney(){
 ...
 const discoveryTasks=(state.tasks||[]).filter(t=>t.package_code==='client_discovery');
 const optionalDiscovery=current.key==='discovery'?`<button class="btn secondary" ${discoveryTasks.length?'data-open="tasks" data-view="client_work"':'data-start-package="client_discovery"'} type="button">${discoveryTasks.length?'Open Client Checklist':'Send Client Checklist (optional)'}</button>`:'';
