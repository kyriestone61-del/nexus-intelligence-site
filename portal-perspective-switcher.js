...
function switcherMarkup(state,mode){
  ...
  return `<summary aria-label="Switch Nexus perspective"><span class="perspective-label">View as</span><b>${esc(MODE_LABELS[mode]||'Admin')}</b><span class="perspective-client">${esc(company?.name||'Select client')}</span></summary><div class="nexus-perspective-menu"><div class="perspective-account"><label for="nexusPerspectiveCompany">Client account</label><select id="nexusPerspectiveCompany">${companies.map(c=>`<option value="${esc(c.id)}" ${c.id===state.companyId?'selected':''}>${esc(c.name)}</option>`).join('')}</select></div><div class="nexus-perspective-options"><button class="nexus-perspective-option ${mode==='admin'?'active':''}" data-perspective="admin" type="button"><span><b>Admin</b><small>Founder controls, diagnosis, approvals, revenue and configuration.</small></span>${mode==='admin'?'<i>✓</i>':'<i>›</i>'}</button><button class="nexus-perspective-option ${mode==='operations'?'active':''}" data-perspective="operations" type="button"><span><b>Operations</b><small>Delivery, QA, agents, workflows, health and operating reviews.</small></span>${mode==='operations'?'<i>✓</i>':'<i>›</i>'}</button><button class="nexus-perspective-option ${mode==='client'?'active':''}" data-perspective="client" type="button"><span><b>Client View</b><small>Read-only preview of what the selected client can see.</small></span>${mode==='client'?'<i>✓</i>':'<i>›</i>'}</button></div><p class="nexus-perspective-note">Switching perspective never changes your real Nexus administrator authorization.</p></div>`;
}
