...
export function mountDiagnosisOffer(root,portal){
  ...
  async function refresh(value){
    ...
    const offer=value.offer,price=new Intl.NumberFormat(undefined,{style:'currency',currency:offer.currency}).format(offer.price_cents/100);
    root.innerHTML=`<article class="relystra-build-card"><h2>Operational Diagnosis · ${esc(price)}</h2><p>Begin after the 15–20 minute fit call. The diagnosis includes structured operational analysis, workflow findings, a missing-information review, pre-build Actions and a preliminary Build roadmap.</p>
      ${offer.checkout_enabled?`<button class="btn primary" data-diagnosis-purchase>${plans.length?'Continue diagnosis payment':'Review & purchase diagnosis'}</button>`:'<p>Relystra is preparing payment access. Your workspace is available while setup is completed.</p>'}</article>`;
  }
  const click=async event=>{
    if(!event.target.closest('[data-diagnosis-purchase]')||busy||snapshot?.company_id!==portal.state.companyId)return;
    ...
    try{
      ...
      const {data,error}=await portal.sb.functions.invoke('relystra-checkout',{body:{plan_id:id,operation:'checkout'}});
      ...
      if(data.url){const url=new URL(data.url);if(url.protocol!=='https:'||url.hostname!=='checkout.stripe.com')throw new Error('Unexpected checkout destination.');location.assign(url.href)}
