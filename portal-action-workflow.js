...
function checklistMarkup(task){
  ...
  return `<div class="embedded-checklist"><div class="embedded-checklist-head"><b>${complete} of ${reqs.length} items addressed</b><span>${complete===reqs.length?'Checklist ready to complete':'Work through the items below'}</span></div>${reqs.map(r=>{
    ...
    return `<article class="embedded-requirement ${addressed(r.status)?'done':''}" data-req-id="${r.id}"><div class="embedded-req-top"><div><b>${esc(friendlyRequirementTitle(c.title))}</b><span>${esc(c.why_needed||'This helps Relystra understand the current state without guessing.')}</span></div><span class="embedded-status">${esc(String(r.status||'needed').replaceAll('_',' '))}</span></div>${r.client_note?`<div class="embedded-answer"><b>Your response:</b> ${esc(r.client_note)}</div>`:''}${!state.admin?`<div class="embedded-req-actions">${fileLike?`<button class="btn primary req-open-upload" data-id="${r.id}" type="button">Upload / provide</button>`:''}${answerLike?`<button class="btn secondary req-inline-answer" data-id="${r.id}" type="button">Answer here</button>`:''}<button class="btn secondary req-inline-build" data-id="${r.id}" type="button">Build with Relystra</button><button class="btn secondary req-inline-na" data-id="${r.id}" type="button">Not applicable</button></div>${answerLike?`<div class="embedded-answer-editor" data-id="${r.id}"><textarea data-req-note="${r.id}" placeholder="A short, clear answer is enough.">${esc(r.client_note||'')}</textarea><button class="btn primary req-inline-save" data-id="${r.id}" type="button">Save response</button></div>`:''}`:''}</article>`;
  }).join('')}</div>`;
}

function workflowEvidenceMarkup(task){
  return `<div class="workflow-evidence-guide"><h4>What “current-workflow evidence” means</h4><p>Show Relystra how the work actually happens <b>today</b>—not how it is supposed to happen in theory.</p><div class="evidence-guide-grid"><div><b>What we are looking for</b><span>People involved, steps, systems, handoffs, repeated work, delays, exceptions, and what a normal completed example looks like.</span></div><div><b>Useful examples</b><span>SOPs, checklists, screenshots, booking/CRM exports, spreadsheets, sample emails, forms, reports, or a short written walkthrough.</span></div><div><b>What you do not need</b><span>You do not need polished documentation or a formal process map. Representative evidence is enough.</span></div></div>${!state.admin?'<button class="btn primary workflow-upload-btn" type="button">Open the secure upload area →</button>':''}</div>${formMarkup(task)}`;
}
...
async function saveTaskResponse(card,{complete=false}={}){
  ...
  toast(complete?'Action completed. Relystra has been notified.':'Response saved. Relystra has been notified.');
...
async function setRequirement(id,status){
  ...
  toast(status==='build_with_nexus'?'Relystra will help build this item.':'Checklist item updated.');await workspace();taskStamp='';
}
...
function enhanceTaskSection(){
  ...
  const p=section?.querySelector('.toolbar p.small');if(p)p.textContent=state.admin?'Assign clear work, review client responses, and keep responsibility synchronized across the engagement.':'Complete your Relystra work here. Each action tells you exactly what is needed and saves your response directly to the shared engagement record.';
}
...
function ensureDiagnosisDraftPanel(){
  ...
  panel.innerHTML=`<div class="toolbar"><div><div class="kicker">Diagnosis → evidence requests</div><h2>Requests to review</h2><p class="small">Relystra creates draft evidence requests from unresolved preparation gaps and, when a diagnosis result supplies structured evidence gaps, from those findings too. <b>Nothing is sent to the client until you approve it here.</b></p></div>${openDrafts.length>1?'<button id="sendAllDiagnosisDrafts" class="btn secondary" type="button">Send all reviewed drafts</button>':''}</div><div class="draft-summary"><b>${openDrafts.length}</b><span>draft request${openDrafts.length===1?'':'s'} waiting for review</span></div><div class="diagnosis-draft-list">${diagnosisDrafts.length?diagnosisDrafts.map(draftCard).join(''):'<div class="empty">No diagnosis-generated requests yet. Queue a diagnosis to create preliminary request drafts from unresolved preparation gaps.</div>'}</div>`;
  bindDrafts();
}
