...
if(state.admin)throw new Error('Client Guide must not load in the Relystra admin workspace.');
...
const rawStatusMap={
  ...
  submitted:'Relystra working',reviewing:'Relystra working',planned:'Relystra working',planning:'Relystra working',in_progress:'Relystra working',active:'Relystra working',building:'Relystra working',queued:'Relystra working',analyzing:'Relystra working',not_started:'Relystra working',draft:'Relystra working'
};
function humanStatus(raw,{owner}={}){
  ...
  return rawStatusMap[key]||'Relystra working';
}
...
const faqs=[
  ...
  {id:'start-nexus',category:'Start Here',q:'How do I know what Relystra is working on?',keywords:'nexus working doing status progress work',answer:'Home shows current Relystra-owned work separately from anything waiting on you.',steps:['Open Home.','Find “Relystra is working on.”','Open Progress only when you want more detail.'],dest:'overview',action:'Show Relystra work'},
  {id:'files-what',category:'Files & Evidence',q:'What should I upload?',keywords:'upload file evidence what documents provide',answer:'Upload only the items Relystra specifically requests. A few representative examples are usually better than a giant data dump.',steps:['Open Files.','Choose the requested evidence item.','Tap Upload file on that item and select the file.'],dest:'documents',action:'Go to Files'},
  {id:'files-missing',category:'Files & Evidence',q:'What if I do not have a requested file or document?',keywords:'missing no file document dont have build nexus',answer:'That is okay. Missing documentation is useful information. Choose “Build with Relystra” when that option is available.',steps:['Open Files.','Find the requested item.','Choose “Build with Relystra” instead of creating something just to satisfy the checklist.'],dest:'documents',action:'Go to Files'},
  {id:'files-safe',category:'Files & Evidence',q:'What should I never upload?',keywords:'privacy safe sensitive password card ssn secret api medical',answer:'Do not upload passwords, API keys, MFA codes, full payment-card data, SSNs, medical records, crypto private keys, or unrelated sensitive information unless Relystra has explicitly approved that data type.',steps:['Review the requested evidence item.','Redact information Relystra does not need.','Upload only the minimum relevant business evidence.'],dest:'documents',action:'Review Files'},
  {id:'files-status',category:'Files & Evidence',q:'How do I know whether Relystra received my file?',keywords:'received upload status file shared received',answer:'The evidence item and Files list show the uploaded file after it is linked successfully.',steps:['Open Files.','Find the evidence item you uploaded to.','Confirm the file appears under that item or under files already shared.'],dest:'documents',action:'Check my Files'},
  {id:'reports-find',category:'Reports',q:'Where is my Relystra report?',keywords:'report diagnosis findings where results pdf',answer:'Released findings live in Reports. Internal Relystra drafts never appear there.',steps:['Open Reports.','Choose the released report.','Read the summary first; expand into details only when needed.'],dest:'diagnosis-reports',action:'Open Reports'},
  ...
  {id:'progress-request',category:'Requests & Actions',q:'How do I ask Relystra for something new?',keywords:'new request ask nexus problem help change automation reporting',answer:'Use Ask Relystra first. If the Guide cannot resolve it, it can prepare a request for the Relystra team without making you hunt for a form.',steps:['Open Ask Relystra.','Describe what you need in normal language.','If needed, choose “Send this question to Relystra,” review the prepared request, and submit it.'],dest:'help',action:'Ask Relystra'},
  ...
  {id:'results',category:'Projects & Progress',q:'Where can I see what improved?',keywords:'results improvement metric baseline current target roi value',answer:'Measured changes appear under Results inside Progress. Relystra keeps baseline, current, and target values together when they exist.',steps:['Open Progress.','Scroll to “Results.”','Compare baseline, current, and target values.'],dest:'progress',action:'See Results'},
  {id:'statuses',category:'Start Here',q:'What do the status labels mean?',keywords:'status waiting nexus working ready review complete blocked meaning',answer:'Relystra uses only five client-facing statuses: Waiting on you, Relystra working, Ready to review, Complete, and Blocked.',steps:['“Waiting on you” means you have the next move.','“Relystra working” means no action is required from you right now.','“Ready to review” needs your attention; “Complete” is finished; “Blocked” needs an issue resolved.'],dest:'overview',action:'Return Home'},
  ...
  {id:'privacy',category:'Account & Notifications',q:'Can another client see my information?',keywords:'privacy tenant company another client see data security',answer:'Your client workspace is company-scoped. Client-visible tools should return only information authorized for your company, while internal Relystra material remains separate.',steps:['Use your authenticated workspace.','Share evidence only through Files.','Contact Relystra immediately if anything appears to belong to another company.'],dest:'help',action:'Open Help'},
  {id:'help-human',category:'Account & Notifications',q:'What if the Relystra Guide cannot answer my question?',keywords:'human support contact nexus guide cannot answer help',answer:'The Guide will offer to prepare a request for the Relystra team. Nothing is sent until you review and submit it.',steps:['Ask the question in Relystra Guide.','Choose “Send this question to Relystra.”','Review the prepared request and tap Submit request.'],dest:'help',action:'Ask Relystra'}
];
...
function clientActions(){
  const items=[];
  arr(state.tasks).filter(t=>String(t.assignee||'').toLowerCase()==='client'&&!['complete','completed'].includes(String(t.status||'').toLowerCase())).forEach(t=>items.push({kind:'task',title:t.title||'Action item',copy:t.description||'Relystra needs you to complete this action.',status:t.status||'waiting_on_client',priority:t.priority||'normal',due:t.due_date,section:'tasks',cta:'Open action'}));
  snapshot.approvals.filter(a=>a.status==='pending').forEach(a=>items.push({kind:'approval',title:a.title||'Decision',copy:a.description||'Relystra needs your decision.',status:'ready_for_review',priority:'high',due:a.due_date,section:'approvals',cta:'Review decision'}));
  snapshot.docRequests.filter(d=>d.status==='requested').forEach(d=>items.push({kind:'file',title:d.title||'Requested information',copy:d.purpose||'Relystra needs this information to continue.',status:'waiting_on_client',priority:'normal',due:d.due_date,section:'documents',cta:'Provide item'}));
  return rankClientActions(items,3);
}
function nexusWorking(){
  const items=[];
  arr(state.tasks).filter(t=>String(t.assignee||'').toLowerCase()==='nexus'&&!['complete','completed'].includes(String(t.status||'').toLowerCase())).forEach(t=>items.push({title:t.title||'Relystra action',copy:t.description||'Relystra is moving this work forward.',status:t.status||'in_progress'}));
  ...
  snapshot.automations.filter(a=>['building','healthy','active'].includes(a.status)).forEach(a=>items.push({title:a.name||'Relystra-supported system',copy:a.purpose||'Relystra is supporting this system.',status:a.status}));
...
function renderHome(){
  ...
  <section class="client-focus-panel"><div class="client-section-head"><div><div class="kicker">Needs you now</div><h2>${actions.length?`${actions.length} ${actions.length===1?'item':'items'} waiting on you`:actionsVerified?'You are clear right now':'Live action state could not be verified'}</h2></div></div>${actions.length?`<div class="client-action-grid">${actions.map(actionCard).join('')}</div>`:actionsVerified?'<div class="client-clear-state"><b>Nothing needs your attention.</b><span>Relystra will surface the next action here when one is ready.</span></div>':'<div class="client-clear-state client-data-warning"><b>Live action state could not be verified.</b><span>Do not assume there is nothing to do. Refresh the workspace before proceeding.</span><button class="btn secondary" type="button" data-client-refresh>Refresh</button></div>'}</section>
  <div class="client-home-grid"><section class="client-soft-panel"><div class="kicker">Relystra is working on</div><h2>No guessing required</h2>${working.length?working.map(x=>`<div class="client-line-item">${statusChip(x.status)}<div><b>${esc(x.title)}</b><span>${esc(x.copy)}</span></div></div>`).join(''):workingVerified?'<div class="client-empty-small">No active Relystra work is visible right now.</div>':'<div class="client-empty-small client-data-warning">Live Relystra work state could not be verified. Refresh before relying on this status.</div>'}</section>
  <section class="client-soft-panel"><div class="kicker">Next milestone</div><h2>${esc(milestone?.title||'No milestone is waiting')}</h2>${milestone?`<p>${esc(milestone.description||'This is the next planned point in the engagement.')}</p><div class="client-meta-row">${statusChip(milestone.status)}${milestone.due_date?`<span>Target ${esc(day(milestone.due_date))}</span>`:''}</div><button class="btn secondary" type="button" data-client-dest="progress">See Progress</button>`:'<p>Relystra will show the next milestone here once it is scheduled.</p>'}</section></div>
...
function faqCard(f){return `<details class="client-faq-card" data-category="${esc(f.category)}" data-search="${esc(`${f.q} ${f.keywords} ${f.answer}`.toLowerCase())}"><summary><span>${esc(f.q)}</span><span aria-hidden="true">+</span></summary><div class="client-faq-answer"><p>${esc(f.answer)}</p><ol>${f.steps.map(s=>`<li>${esc(s)}</li>`).join('')}</ol><button class="btn secondary" type="button" data-faq-dest="${esc(f.dest)}" data-faq-id="${esc(f.id)}">${esc(f.action||'Go there')} →</button><button class="client-ask-inline" type="button" data-guide-question="${esc(f.q)}">Still need help? Ask Relystra.</button></div></details>`}
function filterFaq(){
...
function renderHelp(){
  ...
  root.innerHTML=`<header class="client-page-head"><div><div class="eyebrow">Help</div><h1>Get an answer without hunting around.</h1><p>Search a question, follow three simple steps, or ask Relystra Guide.</p></div><button class="btn primary" id="openNexusGuideFromHelp" type="button">Ask Relystra</button></header>
  ...
  <section class="client-faq-list">${faqs.map(faqCard).join('')}<div id="clientFaqEmpty" class="client-clear-state" hidden><b>No exact FAQ matched.</b><span>Ask Relystra Guide and it will either guide you or prepare a request for the Relystra team.</span><button class="btn primary" id="faqAskInstead" type="button">Ask Relystra</button></div></section>
...
function injectContextHelp(){
  const contexts=[['section-documents','How do I know what Relystra needs from me?'],['section-diagnosis-reports','How do I ask a question about my report?']];
...
function ensureGuide(){
  ...
  const button=document.createElement('button');button.id='nexusGuideButton';button.className='nexus-guide-button';button.type='button';button.setAttribute('aria-haspopup','dialog');button.innerHTML='<span aria-hidden="true">N</span><b>Ask Relystra</b>';document.body.appendChild(button);
  const drawer=document.createElement('aside');drawer.id='nexusGuideDrawer';drawer.className='nexus-guide-drawer';drawer.setAttribute('role','dialog');drawer.setAttribute('aria-modal','true');drawer.setAttribute('aria-label','Relystra Guide');drawer.setAttribute('aria-hidden','true');drawer.innerHTML=`<div class="nexus-guide-head"><div><div class="kicker">Relystra Guide</div><h2>Ask in plain language.</h2><p>I use only client-visible help and workspace context. If I cannot answer confidently, I will help you ask the Relystra team.</p></div><button type="button" id="closeNexusGuide" aria-label="Close Relystra Guide">×</button></div><div id="nexusGuideMessages" class="nexus-guide-messages" aria-live="polite"></div><div id="nexusGuideQuick" class="nexus-guide-quick"><button type="button">What do I need to do?</button><button type="button">How do I upload a file?</button><button type="button">Where is my report?</button><button type="button">What is Relystra working on?</button></div><form id="nexusGuideForm" class="nexus-guide-form"><label for="nexusGuideInput">Your question</label><div><input id="nexusGuideInput" maxlength="1000" autocomplete="off" placeholder="Ask Relystra…"><button type="submit">Send</button></div></form>`;document.body.appendChild(drawer);
...
function dynamicGuide(query){
  ...
  if(/what.*(need|do)|what.*next|my next|needs me/.test(q)){
    const items=clientActions();if(!items.length&&!sourceVerified('approvals','docRequests'))return {html:'<b>Live action state could not be verified.</b><p>Refresh Home before assuming nothing needs you.</p>',dest:'overview',label:'Refresh Home'};if(!items.length)return {html:'<b>You are clear right now.</b><p>Nothing in the verified client-visible workspace is waiting on you. Relystra will put the next required action on Home.</p>',dest:'overview',label:'Open Home'};
...
function askGuide(query){
  ...
  const safe=esc(query);const n=addGuideMessage('bot',`<b>I do not have a confident client-help answer for that.</b><p>I can prepare a request for the Relystra team with your question already filled in. You will review it before anything is submitted.</p><button type="button" class="nexus-guide-action" data-escalate-question="${safe}">Prepare a Relystra request →</button>`);bindMessageActions(n)
}
function prepareRequest(question){
  closeGuide();openDetail('requests','help');setTimeout(()=>{
    const form=$('opsRequestForm');if(form)form.style.display='block';if($('opsRequestCategory'))$('opsRequestCategory').value='other';if($('opsRequestPriority'))$('opsRequestPriority').value='normal';if($('opsRequestTitle'))$('opsRequestTitle').value='Question for Relystra';if($('opsRequestDescription'))$('opsRequestDescription').value=question;form?.scrollIntoView({block:'start',behavior:'smooth'});toast?.('Your question is prepared. Review it, then tap Submit request when you are ready.');
  },120)
}

const tourSteps=[
  {title:'Home tells you what matters.',copy:'Open Relystra and start here. “Needs you now” contains only the actions that actually require you.'},
  {title:'Files is where you give Relystra information.',copy:'Use the requested evidence cards. Upload what you have. If something does not exist, choose Build with Relystra.'},
  {title:'Reports is where Relystra gives you findings.',copy:'Only deliberately released client-safe reports appear here. Questions stay attached to the report.'},
  {title:'Progress and Help finish the loop.',copy:'Progress combines actions, decisions, milestones, and results. Help gives you FAQs and Relystra Guide from anywhere.'}
];
let tourIndex=0;
