export async function onRequest(context) {
  ...
  const enhancements = `\nimport('/secure-documents.js').then(({init})=>init({sb,state,$,toast,download,log,workspace})).catch(err=>console.error('Secure documents enhancement failed',err));\nimport('/portal-auth.js').then(({initAuthUX})=>initAuthUX({sb,$,pane,show})).catch(err=>console.error('Portal auth UX enhancement failed',err));\nimport('/portal-ops.js').then(({initOps})=>{const opsWait=setInterval(()=>{const app=document.getElementById('portalApp'),projectBox=document.getElementById('projectBox');const readyNoClients=state.admin&&Array.isArray(state.companies)&&state.companies.length===0;const readyWorkspace=state.companyId&&projectBox&&projectBox.innerHTML.trim();if(state.user&&app&&app.style.display==='block'&&(readyNoClients||readyWorkspace)){clearInterval(opsWait);initOps({sb,state,$,toast,workspace,log});}},180);setTimeout(()=>clearInterval(opsWait),20000)}).catch(err=>console.error('Portal operations enhancement failed',err));\n`;
