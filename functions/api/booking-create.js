...
export async function onRequestPost(context){
  ...
  try{
    ...
    if(!validApprovedSlot(env,start,end))return new Response(JSON.stringify({ok:false,error:'That booking time is not an approved Nexus appointment slot. Please choose a listed available time.'}),{status:409,headers});
