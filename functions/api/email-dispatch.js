...
export async function onRequestPost({request,env}){
  ...
  async function patchOutbox(id,patch){
    const res=await fetch(`${base}/nexus_email_outbox?id=eq.${encodeURIComponent(id)}`,{
      method:'PATCH',headers:{...supaHeaders,'Prefer':'return=minimal'},body:JSON.stringify({...patch,updated_at:now()})
    });
  ...
  try{
    ...
    for(const row of rows){
      ...
      try{
        ...
        await patchOutbox(row.id,{status:'sent',sent_at:attemptedAt,last_attempt_at:attemptedAt,provider_message_id:payload?.id||null,provider_status:'accepted',provider_event_at:attemptedAt,failure_class:null,last_error:null});
        sent++;
      }catch(error){
