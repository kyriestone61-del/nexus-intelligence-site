...
export function mountTranscriptStage(root,portal,{navigate,onChange=async()=>{}}){
  ...
  function render(){
    ...
    ${transcript?`<p><b>Transcript ready:</b> ${esc(transcript.file_name)}</p>`:'<p>No transcript selected yet. A failed upload will not complete this step.</p>'}
  ...
  async function runDiagnosis(){
    if(busy||!portal.state.admin||!snapshot.diagnosis?.access||!selected())return;
