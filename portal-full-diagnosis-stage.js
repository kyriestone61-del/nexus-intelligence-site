...
export function mountFullDiagnosisStage(root,portal,{navigate,onChange=async()=>{}}){
  ...
  function render(){
    ...
    <section class="relystra-build-card"><h2>${hasResult?'Continue with your diagnosis':processing?'Diagnosis is running':'Run Full Diagnosis'}</h2><p><b>Who acts next:</b> ${admin?'Relystra administrator':'Relystra'}</p>
    ${!access?'<p>Full Diagnosis is included in your first paid engagement. Review its scope, price and payment status to continue; Free Diagnosis and your uploads remain available.</p><button class="btn primary" type="button" data-transcript-setup>Review scope & payment</button>':processing?'<p>The current analysis is in progress. Refresh its status; do not start another run.</p><button class="btn primary" type="button" data-transcript-next>View diagnosis status</button>':hasResult?'<p>The existing diagnosis is ready to revisit. Opening it does not create another AI run.</p><button class="btn primary" type="button" data-transcript-next>Continue to diagnosis & approval</button>':admin?`<p>Analyze the selected transcript together with the authorized supporting evidence. Then review, edit and approve the deeper findings.</p><button class="btn primary" type="button" data-transcript-run ${transcript||retained?'':'disabled'}>${['failed','blocked','revision_requested'].includes(run.status)?'Retry existing diagnosis':'Run Full Diagnosis'}</button>`:'<p>Relystra will run and review the diagnosis. Your released findings will appear in the next step.</p><button class="btn primary" type="button" data-transcript-next>Continue to diagnosis status</button>'}</section></div>`;
    root.querySelector('[data-transcript-setup]')?.addEventListener('click',()=>navigate('commercial-report'));
  ...
  async function upload(event){
    ...
    try{
      ...
      message='Transcript saved. Continue with Run Full Diagnosis below.';failed=false;
      await onChange();
