...
async function companies(){
  ...
  else{const {data:members,error}=await sb.from('nexus_company_members').select('company_id').eq('user_id',state.user.id).eq('active',true);if(error)throw error;const ids=(members||[]).map(x=>x.company_id);if(ids.length){const {data,error:e}=await sb.from('nexus_companies').select('*').in('id',ids);if(e)throw e;state.companies=data||[]}else state.companies=[]}
  ...
  show('portal');$('roleLabel').textContent=state.admin?'Nexus administrator':'Client workspace';$('newMilestoneBtn').style.display=state.admin?'inline-flex':'none';
...
$('onboardForm')?.addEventListener('submit',async e=>{
  e.preventDefault();const button=e.submitter;button&&(button.disabled=true);
  try{
...
async function loadDataRequirements(){
  ...
  const byId=Object.fromEntries(catalog.map(c=>[c.id,c]));state.dataRequirements=(rows||[]).map(r=>({...r,catalog:byId[r.catalog_id]||null})).sort((a,b)=>(a.catalog?.sort_order||999)-(b.catalog?.sort_order||999));
}
...
async function workspace(){
  ...
  const [projects,tasks,miles,metrics,docs,notes,activity,docRequests]=await Promise.all([
  ...
  state.projects=projects.data||[];state.tasks=tasks.data||[];state.miles=miles.data||[];state.metrics=metrics.data||[];state.docs=docs.data||[];state.notes=(notes.data||[]).filter(x=>!x.user_id||x.user_id===state.user.id);state.activity=activity.data||[];state.docRequests=docRequests.data||[];
...
function renderDataRoom(){
  ...
  const requestRoot=$('explicitDocumentRequests');
  const openRequests=state.docRequests.filter(r=>r.status==='requested');
  requestRoot.innerHTML=openRequests.length?`<div class="request-strip">${openRequests.map(r=>`<div class="row"><div class="grow"><span class="pill">Requested by Nexus</span>${r.due_date?` <span class="pill">Due ${date(r.due_date)}</span>`:''}<br><b>${esc(r.title)}</b><div class="small">${esc(r.purpose||'')}</div>${r.examples?`<div class="small"><b>Examples:</b> ${esc(r.examples)}</div>`:''}${r.redaction_guidance?`<div class="small"><b>Privacy:</b> ${esc(r.redaction_guidance)}</div>`:''}<div class="small">If you do not have this item, tell Nexus—we can help create or reconstruct the minimum useful version.</div></div>${state.admin?'':`<button class="btn primary upload-request" data-id="${r.id}" data-title="${esc(r.title)}" type="button">Upload →</button>`}</div>`).join('')}</div>`:'<div class="empty">No additional one-off document requests are outstanding.</div>';
  root.querySelectorAll('.req-upload').forEach(b=>b.onclick=()=>prepareUpload({requirementId:b.dataset.id,title:b.dataset.title}));
  root.querySelectorAll('.req-answer-btn').forEach(b=>b.onclick=()=>{const box=document.querySelector(`.req-answer[data-id="${b.dataset.id}"]`);box?.classList.toggle('open')});
...
function requirementCard(r){
  const c=r.catalog||{},fileLike=['file','export'].includes(c.input_type),answerLike=['answer','list','access_context'].includes(c.input_type);const addressed=addressedStatus(r.status);
  return `<article class="requirement-card ${addressed?'addressed':''}"><div class="requirement-head"><div><span class="pill">${esc(c.category||'Preparation')}</span> <span class="pill">${esc((c.importance||'helpful').replaceAll('_',' '))}</span></div><span class="req-status ${esc(r.status)}">${esc(r.status.replaceAll('_',' '))}</span></div><h3>${esc(c.title||'Preparation item')}</h3><div class="req-detail"><b>Why Nexus needs it</b><p>${esc(c.why_needed||'This helps Nexus understand the current state without guessing.')}</p></div><div class="req-detail"><b>How to find it</b><p>${esc(c.how_to_find||'Ask the person closest to the workflow or check the systems where the work happens.')}</p></div><div class="req-detail"><b>Good examples</b><p>${esc(c.good_examples||'A representative example is enough.')}</p></div><div class="req-detail missing"><b>Don’t have it?</b><p>${esc(c.if_missing||'That is okay. Nexus can help build the minimum useful version with you.')}</p></div>${r.client_note?`<div class="req-detail"><b>Your note / response</b><p>${esc(r.client_note)}</p></div>`:''}${state.admin?'':`<div class="req-actions">${fileLike?`<button class="btn primary req-upload" data-id="${r.id}" data-title="${esc(c.title||'this item')}" type="button">Upload evidence</button>`:''}${answerLike?`<button class="btn secondary req-answer-btn" data-id="${r.id}" type="button">Answer here</button>`:''}<button class="btn secondary req-build" data-id="${r.id}" type="button">Build with Nexus</button><button class="btn secondary req-na" data-id="${r.id}" type="button">Not applicable</button></div><div class="req-answer" data-id="${r.id}"><textarea id="req-note-${r.id}" placeholder="Be specific. A short list or clear explanation is enough.">${esc(r.client_note||'')}</textarea><div class="actions" style="margin-top:8px"><button class="btn primary req-save" data-id="${r.id}" type="button">Save response</button></div></div>`:''}</article>`;
}
...
function prepareUpload({requirementId=null,requestId=null,title=''}){currentRequirementId=requirementId;currentRequestId=requestId;const box=$('uploadContext');box.classList.add('show');box.innerHTML=`<b>Upload for:</b> ${esc(title)} <button id="clearUploadContext" class="btn secondary" type="button" style="min-height:30px;padding:0 9px;margin-left:8px">Clear</button>`;$('docNote').value=$('docNote').value||`Evidence for ${title}`;$('docFile').focus();box.scrollIntoView({behavior:'smooth',block:'center'});setTimeout(()=>$('docFile').click(),300);setTimeout(()=>{$('clearUploadContext')?.addEventListener('click',clearUploadContext)},0)}
...
async function downloadDocument(id){
  ...
  try{
    ...
    const fallback=await sb.storage.from(BUCKET).download(doc.storage_path);if(fallback.error)throw fallback.error;const url=URL.createObjectURL(fallback.data),a=document.createElement('a');a.href=url;a.download=doc.file_name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),2500);
...
$('uploadForm')?.addEventListener('submit',async e=>{
  ...
  const requirement=state.dataRequirements.find(r=>r.id===currentRequirementId),request=state.docRequests.find(r=>r.id===currentRequestId);
  const sensitivity=requirement?.catalog?.sensitivity||request?.sensitivity||'standard';
