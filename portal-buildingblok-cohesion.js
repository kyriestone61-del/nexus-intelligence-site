...
async function loadPortfolio(force=false){
  ...
  try{
    ...
    renderPortfolio();
  }catch(error){console.error('Relystra Companies failed',error);if(root)root.innerHTML=`<div class="note"><b>Companies could not load.</b><br>${esc(error.message||'Refresh and try again.')}</div>`}
...
function guidance(task){
  const type=String(task.task_type||'').toLowerCase(),phase=String(task.phase||'').toLowerCase();let why='Relystra needs this to keep the engagement moving without making assumptions.',provide=task.instructions||task.description||'Complete the action described above.',next='Submit it to Relystra. Relystra will review it, approve it, or tell you exactly what needs revision.';
  if(type==='workflow_evidence'||type==='upload'){why='This evidence verifies how the work actually happens and reduces diagnosis assumptions.';provide=task.instructions||'Upload the requested current-state file or representative example in the Secure Data Room.';next='After the file is uploaded, submit this action to Relystra for review. Uploading alone does not mark the action complete.'}
  else if(type==='approval'){why='This decision requires human authority before Relystra can move to the next controlled step.';next='Submit your decision. Relystra records it in the approval trail and advances only after the required review.'}
  else if(type==='access'){why='The implementation cannot proceed safely until the required access boundary is confirmed.';provide=task.instructions||'Follow the secure access instructions. Never put passwords, MFA codes, API keys, or secrets in Relystra comments.'}
  else if(type==='decision'){why='Relystra needs an explicit business decision rather than inferring one from incomplete evidence.'}
...
async function submitEvidenceTask(task,card,button){
  ...
  try{const {error}=await sb.rpc('nexus_submit_task_for_review',{p_task_id:task.id,p_response_data:response});if(error)throw error;toast?.('Evidence submitted to Relystra for review.');await workspace();setTimeout(enhanceActionCards,80)}catch(error){toast?.(error.message||'Evidence action could not be submitted.')}finally{if(button.isConnected){button.disabled=false;button.textContent=original}}
}
