...
const requestHelp={
  automation:'Use this for a new automation, a workflow improvement, or a repetitive process you want Nexus to simplify.',
...
function simplifyRequests(){
  ...
  const head=root.querySelector('.ops-toolbar .ops-section-copy');if(head)head.textContent='Submit one clear request. Pick the category, explain the business need, and Nexus will route it from there.';
...
async function uploadRequirement(req,file){
  ...
  const policyOk=window.confirm('Upload this file as evidence for this preparation item?\n\nConfirm it is relevant to Nexus work and does not contain passwords, API keys, MFA codes, full payment-card data, SSNs, medical records, crypto private keys, or other restricted information Nexus has not expressly approved.');
  if(!policyOk)return;
  ...
  try{await portal.log?.('requirement_evidence_uploaded','document',data.id,`${state.admin?'Nexus':'Client'} uploaded evidence for ${catalog.title||'preparation item'}`)}catch{}
...
function evidenceFilesMarkup(req){
  ...
  return docs.map(d=>`<div class="vnext-evidence-file"><div><b>${esc(d.file_name)}</b><div class="small">${d.source_role==='nexus'?'Nexus':'Client'} · ${esc(dt(d.created_at))}</div></div><button class="btn secondary vnext-evidence-download" data-doc="${esc(d.id)}" type="button">Download</button></div>`).join('');
}
function enhanceRequirements(){
  ...
  const title=root.closest('.secure-doc-section')?.querySelector('h2')||root.parentElement?.querySelector('h2');if(title&&/Recommended evidence/i.test(title.textContent||''))title.textContent='Evidence that helps Nexus understand the work';
}
...
function improveDiscoveryBrief(){
  ...
  if(!card.querySelector('.vnext-discovery-purpose')){
    const p=document.createElement('div');p.className='vnext-discovery-purpose';p.innerHTML='<b>Purpose:</b> Capture decisions, priorities, constraints, exceptions, ownership, and context that may not be obvious from a transcript or uploaded files. Nexus uses this to reduce assumptions and improve the diagnosis. The brief is saved into the same evidence lineage as the diagnosis—not treated as a separate project.';h2?.after(p)
  }
  const notes=card.querySelector('label[for="intakeNotes"]');if(notes)notes.textContent='What should Nexus know that the transcript/files may not show?';
...
function reportMarkup(release,questions){
  ...
  const qa=q.length?q.map(x=>`<div class="vnext-question"><b>${esc(x.question)}</b><div class="small">Asked ${esc(dt(x.created_at))}</div>${x.answer?`<div class="answer"><div class="kicker">Nexus answer</div>${esc(x.answer)}</div>`:state.admin?`<div class="vnext-admin-answer"><textarea data-answer-for="${esc(x.id)}" placeholder="Answer this client question clearly and directly."></textarea><button class="btn primary vnext-answer-question" data-id="${esc(x.id)}" type="button">Send answer</button></div>`:'<div class="small" style="margin-top:7px">Nexus is reviewing this question.</div>'}</div>`).join(''):'<div class="empty">No questions have been submitted for this report.</div>';
  return `<div class="vnext-report-head"><div><div class="kicker">Released ${esc(dt(release.released_at))}</div><h2 style="margin:5px 0">Nexus Diagnosis Report</h2></div><div class="actions"><button class="btn secondary vnext-client-pdf" data-release="${esc(release.id)}" type="button">Download PDF</button></div></div><div class="vnext-report-summary"><div class="kicker">Executive summary</div><p>${esc(r.executive_summary||'Nexus has released the reviewed findings for this engagement.')}</p></div>${sections}<div class="vnext-qa"><div class="kicker">Questions for Nexus</div><h3 style="margin:5px 0">Ask about this report</h3>${!state.admin?`<form class="vnext-question-form" data-release="${esc(release.id)}"><textarea required maxlength="6000" placeholder="Ask about a finding, recommendation, next step, or anything that is unclear."></textarea><div class="actions" style="margin-top:8px"><button class="btn primary" type="submit">Submit question</button></div></form>`:''}<div style="height:10px"></div>${qa}</div>`;
}
...
function enhanceDiagnosisModal(){
  ...
  tools.querySelector('.vnext-release-report').onclick=async e=>{
    const button=e.currentTarget;if(!window.confirm('Release the reviewed client-safe diagnosis report to this client?\n\nThe full internal Nexus diagnosis remains private. The client will receive a reduced report in their workspace and notification channels according to their preferences.'))return;
