...
function renderAdminIntake(){
  ...
  root.innerHTML=`<div class="admin-intake-banner step2-hero"><div><div class="eyebrow">STEP 4 · FULL DIAGNOSIS & APPROVAL</div><h1>Review evidence and approve the Full Diagnosis.</h1><p>Relystra handles the framework, provenance, gap detection, and agent orchestration behind the scenes. Your job is to review what is known, close material gaps, add context, run the diagnosis, and approve the result.</p></div><div class="step2-client"><span>Client</span><b>${esc(c?.name||'No company selected')}</b><small>${esc(p?.name||'No paid Build Package yet')}</small></div></div>
  ...
  <section class="box intake-card step2-module" data-module="diagnosis"><div class="step2-module-head"><div><div class="kicker">Diagnosis status</div><h2>Run Full Diagnosis</h2><p class="small">Relystra analyzes all authorized evidence, current admin context, and completed client discovery answers. All active discovery documents from Step 1 are included with supporting evidence.</p></div></div>${executionMarkup()}</section>
  <section class="box intake-card step2-module" data-module="review"><div class="step2-module-head"><div><div class="kicker"> Review & Approve</div><h2>Approve the Full Diagnosis that drives the engagement</h2><p class="small">Approval makes this diagnosis the root record for downstream opportunities, action items, requests, measurements, and the recommended first intervention.</p></div></div>${reviewMarkup()}</section>`;
  bindStep2();
}
