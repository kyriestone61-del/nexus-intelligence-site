...
if (state.admin) throw new Error('Client Shell must not load in the Relystra admin workspace.');
...
function ensureShell() {
  ...
  if (!portalApp || !main || !sidebar || !topbar) throw new Error('Relystra client shell anchors are missing.');
...
function updateMiniContext() {
  ...
  if (!host) return;
  host.innerHTML = `<div class="kicker">Client workspace</div><b>${esc(company?.name || 'Relystra workspace')}</b><span>${esc(company?.industry || company?.website || 'Private client workspace')}</span>`;
}
...
function renderHome() {
  ...
  root.innerHTML = `<header class="nexus-client-page-head"><div><div class="eyebrow">Home</div><h1>What do you need to do?</h1><p>One next step. Everything else stays out of the way until it is ready.</p></div><button class="btn secondary" type="button" data-open-guide-question="What do I need to do next?">Need help?</button></header><section class="nexus-client-primary-action"><div class="nexus-client-primary-label">YOUR NEXT STEP</div>${primary ? `<div class="nexus-client-primary-top"><div><h2>${esc(primary.title)}</h2>${primary.dueDate ? `<span class="nexus-client-due">Due ${esc(day(primary.dueDate))}</span>` : ''}</div>${shellStatus('WAITING_ON_YOU')}</div><div class="nexus-client-primary-grid"><div><span>Why Relystra Needs It</span><p>${esc(primary.why)}</p></div><div><span>What You Need to Provide</span><p>${esc(primary.provide)}</p></div><div><span>What Happens Afterward</span><p>${esc(primary.afterward)}</p></div></div><button class="btn primary nexus-client-primary-cta" type="button" data-complete-task="${esc(primary.taskId)}">Complete this step →</button>` : `<div class="nexus-client-clear-state"><b>You are clear right now.</b><span>No dependency-cleared client action is waiting on you. Relystra will place the next required step here automatically.</span></div>`}</section><details class="nexus-client-up-next"><summary><span>UP NEXT — ${secondary.length}</span><span>Show</span></summary><div>${secondary.length ? secondary.map(item => `<button type="button" class="nexus-client-secondary-task" data-complete-task="${esc(item.task.id)}"><span>${shellStatus(item.state)}<b>${esc(item.task.title)}</b></span><small>${item.task.due_date ? `Due ${esc(day(item.task.due_date))}` : 'Ready when you are'}</small></button>`).join('') : '<div class="nexus-client-empty-small">No additional dependency-cleared actions are waiting.</div>'}</div></details><div class="nexus-client-home-grid"><section class="nexus-client-soft-panel"><div class="kicker">Relystra is working on</div><h2>No guessing required</h2>${working.length ? working.map(item => `<div class="nexus-client-line-item">${shellStatus('NEXUS_WORKING')}<div><b>${esc(item.task.title)}</b><span>${esc(item.task.description || item.task.instructions || 'Relystra is moving this work forward.')}</span></div></div>`).join('') : '<div class="nexus-client-empty-small">No active Relystra-owned work is visible right now.</div>'}</section><section class="nexus-client-soft-panel"><div class="kicker">Next milestone</div><h2>${esc(milestone?.title || 'No milestone is waiting')}</h2><p>${esc(milestone?.description || 'Relystra will show the next milestone here once it is scheduled.')}</p>${milestone?.due_date ? `<span class="nexus-client-muted">Target ${esc(day(milestone.due_date))}</span>` : ''}<button class="btn secondary" type="button" data-client-go="progress">See Progress →</button></section></div><section class="nexus-client-soft-panel nexus-client-recent-done"><div class="nexus-client-section-head"><div><div class="kicker">Recently completed</div><h2>What moved forward</h2></div><button class="btn secondary" type="button" data-client-go="progress">See all progress</button></div>${done.length ? `<div class="nexus-client-done-grid">${done.map(item => `<div><span aria-hidden="true">✓</span><b>${esc(item.task.title)}</b><small>${esc(day(item.task.completed_at || item.task.updated_at))}</small></div>`).join('')}</div>` : '<div class="nexus-client-empty-small">Completed client steps will collect here.</div>'}</section>`;
  bindCommon(root);
}
...
function renderReports() {
  ...
  root.innerHTML = `<header class="nexus-client-page-head"><div><div class="eyebrow">Reports</div><h1>Released findings only.</h1><p>Internal Relystra drafts, evidence scoring, agent traces, release queues, review states, and ranking logic never appear here.</p></div><button class="btn secondary" type="button" data-open-guide-question="Where is my Relystra report?">Need help?</button></header>${safeReports.length ? `<div class="nexus-client-report-stack">${safeReports.map(report => `<article class="nexus-client-report"><div class="nexus-client-report-head"><div><span class="nexus-client-status complete">Released</span><h2>${esc(report.title || `Relystra report v${report.reportVersion || 1}`)}</h2><small>${report.releasedAt ? `Released ${esc(day(report.releasedAt))}` : ''}</small></div></div>${Object.entries(report).filter(([key]) => !['id', 'reportVersion', 'releasedAt', 'title'].includes(key)).map(([key, value]) => reportSectionMarkup(key, value)).join('')}</article>`).join('')}</div>` : '<div class="nexus-client-clear-state"><b>No released report is available yet.</b><span>Internal drafts stay private until Relystra deliberately releases a client-safe report.</span></div>'}`;
  bindCommon(root);
}

function taskRow(item, mode) {
  const task = item.task;
  if (mode === 'next') return `<div class="nexus-client-progress-row upcoming"><div>${shellStatus('UPCOMING')}<b>${esc(task.title)}</b><span>Available after Relystra completes “${esc(item.blockedByTitle || 'the required prerequisite')}”.</span></div><small>${task.due_date ? esc(day(task.due_date)) : 'Future step'}</small></div>`;
...
const FAQS = [['What do I need to do next?', 'Home shows exactly one primary action. Progress separates ready work from future work.'], ['What should I upload?', 'Upload only requested representative evidence. Redact anything Relystra does not need.'], ['What if I do not have the requested file?', 'Do not create a fake document. Tell Relystra the item does not exist so the gap can become part of the work.'], ['Where is my report?', 'Released client-safe findings appear in Reports. Internal drafts never appear there.'], ['What is Relystra working on?', 'Home shows a short list of Relystra-owned work. You do not need to act on it unless the state changes.'], ['What does Upcoming mean?', 'The task is known, but one or more prerequisite Relystra steps are incomplete. It is intentionally unclickable until those prerequisites clear.']];

function renderHelp() {
  const root = $('nexus-client-help');
  if (!root) return;
  root.innerHTML = `<header class="nexus-client-page-head"><div><div class="eyebrow">Help</div><h1>Get an answer without hunting around.</h1><p>Ask Relystra in plain language or use the short client FAQ.</p></div><button class="btn primary" type="button" data-open-guide>Ask Relystra</button></header><section class="nexus-client-help-panel"><div class="kicker">Client FAQ</div><div class="nexus-client-faq-list">${FAQS.map(([q, a]) => `<details><summary><span>${esc(q)}</span><span aria-hidden="true">+</span></summary><p>${esc(a)}</p></details>`).join('')}</div></section><section class="nexus-client-help-panel"><div class="kicker">Notifications</div><h2>How Relystra contacts you</h2><div id="nexusClientPreferencesHost"></div></section>`;
  mountNotificationPreferences();
  bindCommon(root);
}
...
function openTask(taskId) {
  ...
  if (evaluated.state !== 'WAITING_ON_YOU') return toast?.(evaluated.state === 'UPCOMING' ? `This step is available after Relystra completes ${evaluated.blockedByTitle || 'the required prerequisite'}.` : 'This step is not waiting on you right now.');
  ...
  body.innerHTML = `<div class="nexus-client-task-brief"><div><span>Why Relystra Needs It</span><p>${esc(task.description || 'Relystra needs this information or decision to move the engagement forward without making assumptions.')}</p></div><div><span>What You Need to Provide</span><p>${esc(task.instructions || 'Complete the requested action and submit the context Relystra needs to review it.')}</p></div></div><form id="nexusClientTaskForm" data-task-id="${esc(task.id)}">${schema.length ? schema.map(field => renderSchemaField(field, existing)).join('') : `<div class="field"><label for="nexusClientCompletionNote">Your response</label><textarea id="nexusClientCompletionNote" data-task-field="completion_note" required placeholder="Add the answer, decision, confirmation, or completion context Relystra needs to review.">${esc(existing.completion_note || '')}</textarea></div>`}${type === 'access' ? '<div class="nexus-client-security-note"><b>Security boundary</b><span>Do not enter passwords, MFA codes, API keys, recovery codes, or private secrets here.</span></div>' : ''}<div class="actions"><button class="btn primary" type="submit">Submit to Relystra →</button><button class="btn secondary" type="button" data-close-client-modal>Cancel</button></div></form>`;
...
async function submitTask(event) {
  ...
  try {
    ...
    await portal.log?.('task_submitted', 'task', taskId, 'Client submitted action for Relystra review');
    closeModal($('nexusClientTaskModal')); toast?.('Submitted to Relystra. The next step will appear only when it is actually ready.');
  ...
  finally { button.disabled = false; button.textContent = 'Submit to Relystra →'; }
}
...
function ensureGuide() {
  ...
  const button = document.createElement('button'); button.id = 'nexusClientGuideButton'; button.className = 'nexus-client-guide-button'; button.type = 'button'; button.setAttribute('aria-haspopup', 'dialog'); button.innerHTML = '<span aria-hidden="true">N</span><b>Ask Relystra</b>'; document.body.appendChild(button); button.addEventListener('click', () => openGuide());
  ...
  drawer.innerHTML = `<div class="nexus-client-guide-head"><div><div class="kicker">Relystra Guide</div><h2 id="nexusClientGuideTitle">Ask in plain language.</h2><p>The Guide uses the same dependency-aware action context as Home and Inbox.</p></div><button type="button" class="nexus-client-icon-button" data-close-client-guide aria-label="Close Relystra Guide">×</button></div><div id="nexusClientGuideMessages" class="nexus-client-guide-messages" aria-live="polite"></div><div class="nexus-client-guide-quick"><button type="button">What do I need to do?</button><button type="button">What is Relystra working on?</button><button type="button">What should I upload?</button><button type="button">Where is my report?</button></div><form id="nexusClientGuideForm" class="nexus-client-guide-form"><label for="nexusClientGuideInput">Your question</label><div><input id="nexusClientGuideInput" maxlength="1000" autocomplete="off" placeholder="Ask Relystra…"><button type="submit">Send</button></div></form>`;
...
function askGuide(question) {
  ...
  if (/nexus.*working|what.*nexus.*doing/.test(q)) { const work = currentContext?.nexusWorking.slice(0, 3) || []; addGuideMessage('bot', work.length ? `<b>Relystra is currently working on:</b><ol>${work.map(item => `<li>${esc(item.task.title)}</li>`).join('')}</ol><p>You do not need to act on these until Home says a client step is ready.</p>` : '<b>No active Relystra-owned work is visible right now.</b>'); return; }
  ...
  if (/report|diagnosis|findings/.test(q)) { const count = releases.filter(item => String(item.status || '').toLowerCase() === 'released' && !item.revoked_at).length; addGuideMessage('bot', count ? `<b>${count} released ${count === 1 ? 'report is' : 'reports are'} available.</b><p>Only client-safe released findings appear there.</p><button type="button" data-guide-go="reports">Open Reports →</button>` : '<b>No released report is available yet.</b><p>Internal drafts remain private until Relystra deliberately releases them.</p><button type="button" data-guide-go="reports">Open Reports →</button>'); return; }
  if (/upcoming|future|blocked|prerequisite|dependency/.test(q)) { addGuideMessage('bot', '<b>Upcoming means the task is known but not actionable yet.</b><p>Relystra has not completed every prerequisite. The task stays muted and unclickable until those prerequisites clear.</p><button type="button" data-guide-go="progress">Open Progress →</button>'); return; }
  ...
  addGuideMessage('bot', '<b>I do not have a confident workspace answer for that.</b><p>Use Help to contact Relystra rather than relying on a guess.</p><button type="button" data-guide-go="help">Open Help →</button>');
}
...
function inboxCard(entry) {
  ...
  if (entry.type === 'approval') return `<button type="button" class="nexus-client-inbox-card" data-inbox-approval="${esc(entry.item.approval_chain_id)}"><span class="nexus-client-status ready-to-review">Ready to review</span><h3>${esc(entry.item.title)}</h3><p>${esc(entry.item.message || 'Relystra needs your decision.')}</p><b>Review decision →</b></button>`;
...
async function refreshClientShell({ force = false } = {}) {
  ...
  refreshInFlight = (async () => {
    try {
      ...
      console.error('Relystra Client Shell refresh failed', error); const root = $(`nexus-client-${activeView}`) || $('nexus-client-home');
      if (root) root.innerHTML = `<div class="nexus-client-data-warning"><b>Relystra could not verify the live client workspace.</b><span>${esc(error.message || 'Refresh and try again.')}</span><button class="btn secondary" type="button" data-shell-retry>Refresh</button></div>`;
